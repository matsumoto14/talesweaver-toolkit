...
- [013 Tale Wiki の取り込み](013-wiki-import.md) — 4 ゴール、取込キャッシュ → 抽出 / 索引 / 整形の層、段階 0→5 の順、本文は持たず要約 + 元ページリンク
- [014 UI の検証方法](014-ui-verification.md) — 実機の自動巡回監査を全廃。人が押して判断し、撮影だけ機械に任せる
- [015 UI の部品化](015-ui-primitives.md) — 呼ぶ側が決める数を 0 に。標準 → 借りる → 自前。段階 1 は `<dialog>` のモーダル
- [016 召喚獣のモデル](016-summon-model.md) — 攻撃者(本体 / 魔法人形)を別軸に。係数は wiki の `STAB(熊)` 行を 1 関数から引き、DPS は攻撃者ごとに出して足す
- [017 敵にかけるデバフと出典の格付け](017-debuff-model.md) — 公式お知らせ > wiki、敵被ダメージ増加は S 項に負値で載せる、敵攻撃ダメージ減少は消費側が無いので収録しない
- [018 武器形態と <フラグ>](018-weapon-form-and-flag.md) — 効く技は効果側でなく技の印で絞る、速剣は技の変種、<フラグ> は式の外で合成し DPS は積み直しの 1 周で出す
- [019 回しで DPS を出す](019-rotation-dps.md) — 連打する技 + 差し込む CT 技、CT ≤ 所要時間は CT なし扱い、既定は火力最大を ON にして下がるなら教える
- [020 wiki に聞く](020-wiki-ask.md) — ゼリッピが出典つきで答える。段階 0 は LLM なしの FTS 検索。unit_fts は contentless + detail='full'(語は重複除去・ソート済み)、箇条書きは項目ごと、縦見出しの表は「項目 | 内容」
- [021 出典の格付けと訂正の層](021-source-grading.md) — 公式お知らせ > アプリの静的データ > wiki。wiki の値は消さず並べる、訂正はデータ(gamedata から書き出し)でコードが当てる、確かさは confirmed / apparent の 2 段
- [022 表示言語](022-i18n.md) — 日本語の文言を鍵にした gettext 方式で韓国語を足す。切り替えは読み込み直し、Rust の固定の表示名も t() に通す、文を組み立てるものはコード + 引数に
