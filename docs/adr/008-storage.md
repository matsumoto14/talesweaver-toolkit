...
あわせて **IndexedDB の埋め直しを 1 本のカーソルにまとめた**。それまで版ごとに
`openCursor()` を分けていたが、IndexedDB は要求を**置かれた順に**処理するので、同じストアに
カーソルを N 本開くと N 本とも「更新前の行」を読んでから順に書き戻し、**最後の 1 本以外の
埋め直しが消える**。v5 の DB を v9 で開くと `summon_skill_id`(v6)と `lumina_corridor`(v8)が
落ちて、そのキャラのタブが開けない行ができていた(v9 を足して 6 本目になったところで発見)。
1 本にまとめたうえで版ごとの分岐も外し、`migrate_*` と同じく「起動のたび全部走っても同じ結果に
なる」形(欠けている欄だけを埋め、既に今の形の行は書き戻さない)にした。

### v19: 回しに差し込む CT 技を保存する(2026-09-21)

回し(連打する技 1 つ + 差し込む CT 技)のうち、**どの CT 技を差し込むか**をキャラに保存する
`rotation_skill_ids TEXT`(JSON 配列、NULL 可)を追加。3 つの状態を区別する:

- `NULL` = 未設定。`domain::choose_rotation` が選ぶ既定(同形態で `Skill::power` 最大の CT 技 1 つ)
- `[]` = 明示的に差し込まない(連打だけ)
- `["id", …]` = 明示指定

**自動値は保存しない。** 既定 ON の技はキャラタブのチップが点いて見えるが、保存されるのは
ユーザーが触ったときだけ(ux-guidelines「入力欄は自動値を上書きする例外操作」)。自動値を
書き込むと、あとで既定の規則を直しても古い自動値が残って効かなくなる。

SQLite は `SCHEMA_VERSION` 18 → 19(列追加のみ。既存行は NULL のまま)、
IndexedDB(`browserStore.ts`)は 10 → 11 で既存行に `rotation_skill_ids: null` を埋める
(埋め直しは既存の 1 本のカーソルに相乗りする。下の IndexedDB v9 の節を参照)。

書き出し JSON(`transfer.ts`)は欄を足すだけで `FORMAT_VERSION` は上げない —
欄の無い旧い書き出しは `null` = 未設定として読めばよく、上げると読めるファイルを
理由なく拒むことになるため(v17 / v18 の正規化と同じ判断)。

検証(`commands::validate_rotation_skills`)は主軸スキル・召喚スキルと同じ形で、
「そのキャラが自分で撃つ攻撃技」かつ「CT を持つ」ことだけを見る。主軸の**武器形態**を
変えると候補が入れ替わるので、画面側(`StatusPane.svelte`)が形態の変化で未設定に戻す
(保存値を計算時に黙って無視する形にはしない)。
