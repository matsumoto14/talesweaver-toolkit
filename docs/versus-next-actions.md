...
追記(2026-09-02 夕): 対人タブ自体を **方向ごとに 1 列**に作り直し、「次にできること」は
**手ごとに「試す」**を入れると率に反映される形にした(3 段の区分行・開閉は無くし 2 段に戻した)。
手を当てる API は `growth_apply::apply_growth_action`、`preview_versus` は伸びしろを試す前で固定し
率は試した後で返す。その後のレビューで 頭が Picker・命中P / 回避P の開閉ブロック・手は 4 区分
(ステータス / バフ / 装備補正 / エンチャント)の名詞チップ に直した。決定は ADR 007(画面)と ADR 002(domain)。ワイヤー(検討した 3 案と決定案):
https://claude.ai/code/artifact/74256053-6b6c-48e4-aba2-6cc87963394c
