...
- **モデル**: `Character`(素ステ・装備・スキル構成・覚醒/エタ・バフセット)、`Equipment`(部位別 12 スロット + 部位別シエナ登録 + 称号 + テシスコア + アバター強化 + 装備研磨。装備部位はアイテム参照/カスタム・基本能力値・エンチャント・強化 Lv・武器アビリティ・**ランダムオプション**を持つ。シエナのオーラは抽出・注入できるため装備登録と独立した部位別登録一覧 + 装着中IDで持ち、装着中だけを装備攻撃力等へ集計。アバター強化は兜/頭/体/脚/エフェクトの5部位に固定値を付与でき、強化能力値へ合流する。補正付きアバター(末尾「Ａ」)は部位ごとの ON/OFF で持ち、1点で装備補正9値すべてに+1、5部位そろうとセット効果でさらに+10。装備研磨は部位ごとに能力値1つを基本能力値側で上げる消耗品の記録で、バフ「装備研磨」の ON/OFF で実際に合流させるかを切り替える)、`SoulLinkStatus`(リンクステータス 1〜10 の Lv。キャラ単位・条件達成済み前提。1〜7を基本能力/戦闘計算へ接続し、8〜10は記録)、`CommonSkills`(共通スキル。装備攻撃力強化倍率・装備防御力倍率・割合追加ダメージ)、`Skill`、`Buff`、`Enemy`、`Content`(入場条件)
...
- **回し**(`rotation`): 連打する技 1 つ + 差し込む CT 技 0〜数個で DPS を出す(docs/adr/019)。役割の決め方・挟む回数・間隔・DPS・差し込みの損得の規則だけを持ち、1 回ぶんのダメージ計算は呼び出し側(commands / content_evaluation)がする。計算タブもホームの到達一覧もこの 1 本を通る
...
- **インクリ**(`inkri`): 装備のインクリ回数を 1 ずつ積み上げる試行。6 種類のうちビアヌのインクリとエタインクリは失敗しても装備が破壊されず、ビアヌは成功率もそれまでの成功回数で下がる。エタインクリはエタレベル装備(セイクリッド系)専用で、SEED のほかに呪文書を 1 回 1 枚使う。1 回・N 回・成功するまでの試行は domain 内蔵の決定的 PRNG(splitmix64、シード固定で再現可能)で回し、結果は「n 回目の成功までに何回かかったか」の段(`InkriStep`)に割って返す。ゲームが成功のたびに減らす合成回数は追わない(装備側の残量は利用者が知っている。2026-09-18)
- 制約:
...
- **`pages/calc/`**: `CalcPage.svelte`(外枠)— 対象プレート(◀▶ + エリア別一覧)、スキル選択、鎖(1 発 → 合計 → DPS → 討伐時間)、もし〜だったら / `WhyPanel.svelte` — なぜこの数字?(攻撃力の内訳 → 防御を抜く → 倍率で伸ばす、上限で捨てている量、倍率の材料)/ `TracePanel.svelte` — 詳細トレース / `DefensePanel.svelte` — 防御側 / `DetailRows.svelte` — 押した数値の内訳 1 行(鎖・帯・次の候補で共有)/ `MaterialsPane.svelte` — 右カラム「計算の材料」(試し変更の帯と印・極限・覚醒・シャープネス・ソウルリンク・装備切替・エンチャント・研磨・称号・コンボ)/ `BuffCard.svelte` — バフ。段の組み立ては `damageDetail.ts`、開いた面と「何が変わったか」は `detailStore.svelte.ts`、試し変更(KNOBS・同時 3 件まで・保存)は `simStore.svelte.ts`
