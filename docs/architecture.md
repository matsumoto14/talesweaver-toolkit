...
- **インクリ**(`inkri`): 装備の合成回数を消費してインクリ回数を増やす試行。5 種類のうちビアヌのインクリだけ失敗しても装備が破壊されず、成功率もそれまでの成功回数で下がる。1 回・N 回・成功するまでの試行は domain 内蔵の決定的 PRNG(splitmix64、シード固定で再現可能)で回す
- 制約:
...
- `inkri`: ビアヌのインクリ対象装備カタログ(合成回数上限はクライアント DB `c51_995dd080`、ビアヌ費用は wiki の表)。生成は `tools/gamedata/import_inkri_targets.py`
