...
- **モデル**: `Character`(素ステ・装備・スキル構成・覚醒/エタ・バフセット)、`Equipment`(部位別 12 スロット。部位ごとにアイテム参照/カスタム・基本能力値・エンチャント・強化 Lv・武器アビリティを持ち、装備攻撃力・入場条件は集計値で計算)、`Skill`、`Buff`、`Enemy`、`Content`(入場条件)
- **計算**: 能力値計算 → カテゴリ集計 → 与ダメージ式 → 段数・追加ダメージ の4段パイプライン(docs/damage-formula.md §11)
