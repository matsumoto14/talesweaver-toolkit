...
| 7 | `stat_sources.rs:1384-1424` と `:1495-1558` | 寄与の定義が 2 つ(生値の層ステップ幅 / 上限込み累積再構築)。前者は「実質の影響ではない」と 20 行の注意書き付きで `StatPreview` に返す | 累積再構築 1 本にし `StatContribution.effect` を廃止 |
