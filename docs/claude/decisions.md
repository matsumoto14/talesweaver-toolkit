...
- **`W` の供給源が wiki で見つかっていない**(アビリティ「ダメージ増加」+5〜7,000)。
  器・式・上限は揃っているのでデータだけ
- **装備アイテムそのものの効果に器が無い** —
  `X5` 装備エフェクト +3% / AF「全攻撃ダメージ増加」、
  `X6` コラボ装備(武器・手・エフェクト)+3〜5%、
  `Old` 体装備 †主天使の羽 / †シグマウィング +25%、
  `O` 鎧「リナの服」+3%。`EquipmentItem` に同じ `damage_effects` を足す形で入る
