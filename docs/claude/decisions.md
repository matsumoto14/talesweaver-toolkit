...
2. **装備本体はカタログ上限を自動設定し、例外編集だけ許可する** / 装備本体とエンチャントは同じ9値の行で横に並べる。エンチャント枠は `wiki上限 - カタログの装備本体最大` で確定する装備固有値であり、実物の装備本体を編集しても変えない。改・セイクリッドブレードは `840 - 530 = 310`、実物が520なら結果は830
...
10. **レリックの強化段階は直接選択に加えて±1操作も置く** / 現在段階の微調整を10択から探し直させない。−/+は固定位置に常設し、+1と+10の端だけ無効化する
