...
3. `[更新済 → 2026-08-25 装備強化のレンジ倍率 #1]` **武器強化の追加固定ダメージ(§5)= `INT(INT(補正)×倍率)`、奇数なら−1、per-hit は `INT(追加/Hit数)×Hit数`。補正は武器系統ごとの一次式(6 系統、装着アビリティ除外)。+12〜15 はレンジ振りのため実測値の上書き(既定はレンジ下限)** / wiki「装備システム/装備強化」(取得 2026-08-24)をそのまま転記。与ダメージ式の外なのでカテゴリに入れず、`calculate_damage` が per-hit に加算しトレース末尾に段を足す / `crates/domain/src/equipment.rs::weapon_added_damage`・`damage.rs` の Hit 分割テスト(wiki 例 2488/9hit→276)
...
5. **`EQUIPMENT_VALUE_MAX = 9999` の `[仮]` は外さない** / wiki は装備ごとの「上限」行しか持たず、全装備共通の上限は未記載。カタログ取り込み時に `上限 - 装備本体レンジ最大` を固定の `enchant_caps` へ変換する。9999 はカタログ外のカスタム入力に掛ける安全域として残す
...
- 装備ペイン: 鎧に †アビスアーマーを選ぶと装備本体に wiki レンジ(物防 260–280 / 魔防 230–260 / 回避 100–120)を設定する。wiki上限とレンジ最大の差から固定枠を物防30 / 魔防30 / 回避30とし、実物の装備本体値には連動させない
...
## 2026-08-25 ボリスのスキルを wiki から取込(docs/claude/goals/2026-08-25-wiki-gaps.md 段階7)

出典: wiki Skill/ボリス「スキル性能一覧」(一覧表対応バージョン 2026/1/28、取得 2026-08-25)。

1. **旧リポ値との差分はゼロ** / 攻撃力(倍率)・段数・Cri倍を wiki 一覧表と突き合わせた結果、5 件すべて一致(99%/2x、109%/2.5x、113%/2.25x、545%x11/2.7x、550%x11/2.5x)。`SKILLS_SOURCE` を wiki に差し替え、`[仮]` を外した
2. **スキルに 属性 / スキル命中 / Cri値 / SLv を足す** / 一覧表に列がある。属性列で段階3 の `None` 2 件(極・横斬り・極・残影斬)が**無属性**と確定した(属性システムのキャラ表では刀系の一部しか列挙されておらず読み取れなかった)
3. **スキル命中は wiki 表記 +15 した実値を持つ** / wiki 計算式まとめ `#AccuracyPoint` の「当Wikiのスキル命中は実際の数値から15引いた値が記載されているため+15する必要がある」。表示のたびに +15 するとどこかで漏れるので、データ側で 1 回だけ足す
4. **命中P を実装した(段階1 の積み残しを解消)** / 装備命中率補正(段階2)とスキル命中(段階7)が揃った。依存ボーナス/ペナルティは wiki 計算式まとめの依存表を `AccuracyCorrection`(係数は gamedata、評価は domain)にした。命中P増加(バフ)・割合増加(集中/的中剣)・割合減少(感電/雷電)・ランダムOP はカタログに無いので中立値 / `命中pはdexと装備命中とスキル命中から依存ペナルティを引く`
5. **命中「率」は出さない** / 相手の回避Pが要る。狩り場情報一覧の必中命中P(= 敵回避P + 100)がある敵なら出せるが、収録済み 42 体のうち値があるのはごく一部。命中P の実数を出して wiki の必中命中P と見比べてもらう形にした
6. **Cri値(スキルクリティカル率)は保持するだけ** / クリティカル率の式には対象AGI・対象のクリティカル被撃率も要る(§9)。値を捨てると後から復元できないので持つ

### 実機確認(docs/screenshots/99)

- スキル行が `×0.99 ・ 1段 ・ Cri×2 ・ 無属性 ・ 命中P 291`(DEX199 + スキル命中98 − [(STAB1000+HACK215)/200] = 291)

## 2026-08-25 goal レビュー反映(属性の供給源 / 通常回避の打ち切り)

ユーザー確認(2026-08-25)。

1. **Mob からの回避「率」は出さない(U6 とは別の確定)** / 敵の命中Pが取れない以上、上限 85% を前提にした通常回避・最終被弾率は「上限に張り付いた場合」の仮定値にしかならない。防御タブは**回避P(物理/魔法/複合)+ 特殊回避**だけにし、`normal_evasion` / `hit_taken_rate` / `NORMAL_EVASION_CAP` と `DefenseProfile` の 2 フィールドを削除した(式は docs/damage-formula.md §7 に残す)
2. **属性値の供給源が確定した(U6 解消)** / ユーザー提供: 装備の属性強化は**盾+(カフス)とレリックを除く 10 部位 × 9 = 90**、加えて ペット+10 / モンスターカード+30 / ルーンスキル+20 / 頭アビリティ+20 / カフスアビリティ+30。合計 200 + キャラ基礎(最大 10)で 210 になり、敵属性値 120 に対して属性差 +90 = カテゴリI の上限(+50%)に届く。`allows_element` の除外部位(盾+ / レリック)は元から正しかったので実装変更なし
3. **装備以外の供給源は補正源に「属性」を新設して持つ** / 加算値は固定なので入力は「どの属性に乗せているか」の Select だけ。5 供給源の値は gamedata(`element_source_catalog`)、選択は `StatSources::elements`(storage は serde default で migration 不要)/ `供給源は選んだ属性にカタログの値を足す`、`供給源は5件で合計200まで積める`
4. **属性ペインに内訳表を出す** / キャラ基礎 / 装備の属性強化 / この画面の供給源 / 合計(上限 255)の 4 行 × 8 属性。合計だけ出すと「どこを伸ばせば届くか」が読めない / `preview_elements` コマンド
5. **目安ダメージは 1 ヒット最大のまま・上書き UI は作らない** / ユーザー確認。多段スキルが有利になるのを避ける定義。値を直すときは gamedata を編集する
6. **wiki にしか無い敵(S6)は収録しない** / ユーザー決定。今の 42 体で足りる

### 実機確認(docs/screenshots/100〜101)

- 属性ペイン: 供給源 5 件をすべて「水」にすると `この画面の供給源 水 110` / `合計 水 120`(キャラ基礎 10 込み)。装備の属性強化 90 を足すと 210 になる
- 防御タブ: 回避ブロックが 回避P 3 種 + 特殊回避 の 4 行になり、「通常回避(上限)」「最終被弾率」は消えた

## 2026-08-25 全キャラのスキル取込(S5)

出典: wiki 各キャラの `Skill/<キャラ名>` ページ「スキル性能一覧」(取得 2026-08-25)。19 キャラ 303 件。

1. **表をパーサで機械的に取り込む** / 19 ページ × 十数〜数十行を手写しすると転記ミスが混ざる。ヘッダ行(`|スキル|区分|依存|物/魔|属性|…`)から列位置を引き、`~`(前行を継承)を解決して 1 行 = 1 スキルにした。ボリスの 5 件が旧リポ(Excel v4.00 由来)の倍率 / 段数 / Cri倍と**完全一致**したのでパーサを信頼した / `ボリスの5件は旧リポの値と一致する`
2. **id は wiki のページ内アンカーを snake_case にしたもの** / `[[†極・突き>#butt]]` → `lucian_butt`。ローマ字を人力で決めると 303 件でぶれる。**旧 id(`boris_goku_yokogiri` 等)は変わる**ので、保存済みキャラの `main_skill_id` は選び直しになる(検証用 3 キャラのみ。CLAUDE.md「後方互換性を保たない」)
3. **スキル命中・Cri値は `Option<i64>`** / wiki の表が `-` の行が 28 件ある(イェフネンの武器別スキルなど)。0 と未記載を区別する。命中Pは `Skill::accuracy` が `None` なら出さない(`DamageResult::accuracy_point` も `Option`)
4. **`Skill::element` を `Option` から必須に変える** / スキル性能一覧に属性列があり、303 件すべてに値が入っていた。段階3 で `None` にしていた 2 件も埋まったので、`None` を持つ理由が無くなった
5. **除外したもの**(合計 95 行)
   - 銭投げ(SEED 依存、38 行) — 攻撃力が `180 + 120 × スキルレベル` で式が別枠(wiki 計算式まとめの脚注)
   - 同じアンカーの変種行(48 行) — `(瞬撃)` / `(連撃)`(計算式まとめ #ComboSkillType)、`(速剣適用時)`、`(dot)` など。同じスキルの発動違いなので先頭行だけ採る
   - 未対応の依存(9 行) — STAB+INT(ジョシュア 1 / ランジエ 3)、HACK+MR(ベンヤ 3)、INT+STAB+HACK(マキシミン 2)。wiki の「主要ではないタイプ」表にはステ係数と装備係数が載っているが、**装備係数の基本 / 強化の内訳が無い**(`EquipmentCoefficients` は 2 段に分かれる)。INT+STAB+HACK はステ係数が 3 項で `AttackCoefficients`(2 項)にも入らない
6. **スキル Lv 別倍率は未対応のまま** / 表は基本攻撃 SLv1 / 極 SLv10 の 1 点しか載せていない。`Skill::level` にその Lv を持たせて、どの Lv の値かは分かるようにした
7. **アナイスの召喚獣スキルは本体の表に無い** / 別テーブルにあり、召喚獣は本体と別に攻撃力・命中を持つ。25 件は取れたが、召喚獣まわりは別途

### 実機確認(docs/screenshots/102〜103)

- ボリスのスキルピッカーが 5 → **16 種**。合計ダメージ降順で 極・残影斬 / 極・連 / 極・氷撃斬 …(覚醒 0 なのでダメージ上限 7,000 × 段数で頭打ち)
- スキル未収録だったロアミニで `†極・ダークネス・フレア ×1.04 ・ 1段 ・ Cri×2 ・ 黒属性 ・ 命中P 425` が出る

## 2026-08-25 装備強化のレンジ倍率

1. **+12〜+15 の追加固定ダメージは、上書きが無いときレンジの「上限」で自動計算する** / ユーザー決定(2026-08-25)。wiki 装備システム/装備強化の「強化数値の再設定」に**装備強化再設定呪文書で振り直せる**とあり、実用上の想定値はレンジの最上値。下限は「振り直す前の最悪値」で、火力の目安としては低すぎる。これは 2026-08-24 部位別装備モデル #3 の「既定はレンジ下限」を差し替える / `commands::weapon_added_damage`、`レンジ倍率帯は上書き優先_無ければレンジ上限`(+12 上限 280 → 588,280 / +15 上限 880 → 1,848,880)
2. **「等級」はこの話だった** / v4 モックの「最下〜最上」は装備の基本補正値の等級ではなく、**装備強化 +12 以上のランダム値がレンジのどこに入るか**を指していた(ユーザー指摘 2026-08-25)。基本補正値の等級は wiki に無い(2026-08-24 の調査どおり)ので、`EquipmentPart::base` のレンジ + 上書きのままでよい。レンジ倍率を上限にしたことで「最上を引いた状態」が既定になる

## 2026-08-25 ランダムオプション(段階2)

出典: wiki「ランダムオプション」(取得 2026-08-25)。部位ごとの節 + 転移の説明。

1. **ランダムOP は装備補正 9 値に乗らない。与ダメージ式のカテゴリと命中P/回避P に直接効く** / wiki の一覧を読むと効き先が 3 系統に分かれていた。`EquipmentValues` に足すのは誤り
   - 「突き/斬り/物理複合/魔法/神聖/魔法斬り 攻撃力が X% 増加」(盾・レリック右)= **カテゴリP 特定依存ダメージ増加**。wiki の補足列に「特定のスキルの与ダメージが X% 増加 / 突き:STAB依存 / 斬り:HACK依存 / 物理複合:STAB+HACK依存 / 魔法攻撃:INT依存 / 神聖:MR依存 / 魔法斬り:HACK+INT依存」と明記されており、`SkillDependency` の 6 種と 1:1 で対応する。スキルの依存が一致したときだけ乗る
   - 「攻撃ダメージが X% 増加」(盾・レリック右)= **カテゴリX**
   - 「命中率 / 回避率 / 両方が X 増加」(手・レリック左)= 命中P・回避P への加算(wiki `#AccuracyPoint` 末項「ランダムOP」、`#EvasionPoint` の「回避P増加」)
2. **枠数では縛らず、「同じカテゴリー番号は 1 部位に 1 つまで」で縛る** / 部位ごとの枠数は wiki に記載が無い(Item ページの「スロット」列は RO 枠数ではない)。代わりに転移の説明に「同じカテゴリーのオプションを共存させることは出来ず、転移させると優先的に上書きされる。ただし、カテゴリーなし(一覧表では 0 表記)はその限りではない」とあり、これが唯一の wiki 由来の制約。盾のカテゴリー15 は依存別 6 種 + 攻撃ダメージ増加の 7 択で、**このうち 1 つしか付かない** / `同じカテゴリーのランダムオプションは1部位に1つまで`
3. **効果値の既定はレンジ上限、上書きは例外操作** / オプション変化石・再設定で振り直せるので想定値は最上値。装備強化 +12 以上と同じ扱い(2026-08-25 装備強化のレンジ倍率)。ランクを変えるとレンジが変わるので上書きは自動で外す
4. **収録範囲は火力・命中・回避に関係する OP だけ(28 件)** / wiki の一覧は HP/MP/移動速度/経験値/変身/効果音まで含む数百件で、全部入れても計算にも記録にも使えない。ユーザーが「装備を選ぶ」感覚で探せる粒度に絞った
5. **効き先が計算に届かない OP は `RandomOptionEffect::RecordOnly` で入れて「記録するだけ」と明示する** / 破線の枠 + 効き先ラベル + 理由(`note`)を出す。黙って 0 にしない
   - 武器のカテゴリー1(後方から / 一般ボス / レイドボス / 近接)— 追加ダメージ「新-割合」だが発動条件が計算対象に無い
   - 鎧・レリック左のダメージ耐性(カテゴリU)— 被ダメージ計算そのものが無い
   - 盾・鎧の「固定回避」、手の「最大回避率」— §7 の最小回避率補正。通常回避「率」を出していない(2026-08-25 ユーザー決定)
   - 盾+ の「スキルの中ディレイが減少」— 中ディレイが未実装(段階5 で使う)
6. **wiki の節名と部位の対応を確定した** / 「サブアーム」= 盾(`Shield`)、「サブアーム(SHOW)」= **盾+ / カフス**(`ShieldPlus`。盾＋ページの説明が「Show 装備だが補正が設定されている装備」で一致。†ライゾシンボリックカフスの OP 列も 1)。goal の「カフス(盾+)ランダムオプション −1〜−3%」はこの節のこと
7. **レリック(右)(左)は 1 つの `Relic` 部位に統合した** / domain の `PartSlot` はレリックを 1 枠しか持たない。収録した OP のカテゴリーは 右=15 / 左=3・10 で**重ならない**ので、1 部位に統合しても「同じカテゴリーは 1 つまで」の制約が正しく働く / `relic_right_and_left_categories_do_not_collide`
8. **カタログの解決は呼び出し側(commands)が行う** / `Equipment::random_option_totals(&[RandomOptionDef])`。`base_totals(&[EquipmentAbilityDef])` と同じ依存方向(domain は gamedata に依存しない)
9. **DB migration は不要** / `equipment` 列は JSON blob で、`EquipmentPart::random_options` は `#[serde(default)]`。既存行は空配列として読める

## 2026-08-25 称号(段階3)

出典: wiki「称号/normal」「称号/special」(取得 2026-08-25)。

1. **装備枠 1 つで、表示中の 1 件だけが効く** / ユーザー確定(2026-08-25)。所持している称号の累積ではない。`Equipment::title: Option<String>` に id を 1 つ持つ
2. **効き先は装備の基本能力値への加算** / wiki の表の列は 名称 / 習得Lv / 突き / 斬り / 物防 / 魔攻 / 魔防 / 命中 / 回避 / 敏捷 / Cri で、**装備補正 9 値と同じ顔ぶれ**(並びだけ違う)。`EquipmentValues` をそのまま器に使い、`Equipment::base_totals` で合流させる
3. **`PartSlot` を 13 個目に増やさない** / 称号にはエンチャント・装備強化・属性強化・シエナのオーラ・ランダムオプションが無い。部位にすると `EquipmentPart` の affordance を全部持ってしまうので、`Equipment` 直下の独立フィールドにした
4. **収録基準は「補正値 9 種の合計が 15 以上」で 72 件**(主要称号のみ。ユーザー決定 2026-08-25) / 称号は 1 枠しか装備できないので、それ未満は選択肢として意味を持たない。全 565 件を入れても選ぶ手間が増えるだけ。内訳は マーキュリアル洞窟のボス別 30 件(5 ボス × 6 依存、各 +20)、アークロン要塞 7 件、ルーン 6 件、星の戦場 5 件、剣の才能 4 件 ほか
5. **wiki の表はパーサで機械的に取り込む** / 565 行を手写しすると転記ミスが混ざる(全キャラスキル取込と同じ判断)。ヘッダ行から列位置を引き、`&color(){}` / `%%取消線%%` / `&br;` を落として 1 行 = 1 称号にした
6. **id は romaji snake_case**(`eclipse` / `arklon_guardian_stab` / `mercurial_serion_stab`)/ gamedata の enemies・contents と同じ規約。マーキュリアル洞窟のボス別 30 件は `mercurial_<ボス>_<依存>` で規則的に振れる
7. **備考の条件付き効果は計算に入れない** / 「喪失の島関連マップで追加ダメージ+20%」等は §5 の追加ダメージ「新-割合」だが、発動条件(マップ)を計算対象に持っていない。`note` に残すだけ(ランダムオプションの `RecordOnly` と同じ扱い)
8. **グループボーナスはスコープ外**(goal S15)/ 42 グループの「N 個完成で +α」は所持状況 565 件の入力が要る
9. **計算画面の「未実装の補正源」注記が消えた** / 称号・ランダムOP が両方入ったので、残るのは「キャラタブで選んだものが入る」という案内だけになった

## 2026-08-25 共通スキルと装備防御力倍率(段階4a)

出典: wiki「Skill/共通」(取得 2026-08-25)、「装備システム/シエナのオーラ#追加オプション一覧」。

1. **共通スキルを独立した補正源にした(`CommonSkills`)** / goal の指摘どおり「共通スキルが補正源として未実装」が根本だった。装備でもバフでもないので `StatSources` にも `Equipment` にも入らない。効き先は 3 系統: 装備攻撃力強化倍率 / 装備防御力倍率 / 割合追加ダメージ
2. **パワーウェポン・ストロングウェポンを `Equipment` から `CommonSkills` へ移した** / どちらも wiki Skill/共通 のスキルで、装備ではない。`Equipment::enhance_rate()` は `CommonSkills::equipment_attack_rate()` になった。**DB は v6 マイグレーションで `equipment` 列の JSON から `common_skills` 列へ移す**(`migrate_weapon_skills_to_common`)。移行時はストロングウェポン Lv を取れるだけのオーグメント Lv を補う(検証で弾かれて保存できなくなるのを避ける)
3. **オーグメントは倍率ではなく Lv 上限の制約として持つ** / wiki の該当行に「Lv2 以降は共通スキル『オーグメント』の LvUp が必要」と赤字で明記。ストロングウェポン / プロテクトアーマー / ハイパーリミットが対象。`augment_gated_level_max() = augment_level + 1` で検証し、UI の選択肢もそこで絞る
4. **`EQUIPMENT_DEFENSE_RATE = 1.0` 固定を外した** / コートアーマー(物18%/魔12%)+ プロテクトアーマー(Lv1〜6 物36〜81%/魔24〜54%)+ 改・プロテクトアーマー(Lv1〜5 物9〜45%/魔6〜30%)+ シエナのオーラ「防御力増加」を**加算**する。コートアーマーとプロテクトアーマーは wiki に「重複可」と明記
5. **シエナのオーラの追加オプションを 2 つ足した** / 「防御力増加」(1 部位 0〜10%。wiki 備考「実際は装備防御力倍率増加でプロテクトアーマーなどと加算」)と「中ディレイ減少」(0〜2%。段階5 で中ディレイ倍率B へ繋いだ)
6. **シャープネスビジョンは合計ダメージに乗せる** / §5 の「新-割合」は算出基準が「合計ダメージ + 追加ダメージ(武器強化)」。1 段ごとではないので `DamageResult::total` にだけ足し、内訳は `added_damage` / `added_damage_rate` で返す。Lv1〜10 = 5/10/15/20/25/28/31/34/37/40%
7. **リンゴの島・ベリネンルミの 100% 固定は呼び出し側の責務にした** / `defense_profile` は倍率を引数で受け取る。防御タブは対象コンテンツを取らないので習得どおりの倍率で出し、注記で「リンゴの島・ベリネンルミでは常に 100%」と明示する

### 実機確認(docs/screenshots/108)

- v5 → v6 マイグレーション: 既存 4 キャラすべてでパワーウェポン / ストロングウェポン Lv6 が `common_skills` へ移り、`equipment` 列から消えた
- 装備防御力倍率 物199%(コートアーマー + プロテクトアーマー Lv6)で物理防御力 1,263 → 2,510
- シャープネスビジョン Lv10 で 1 発は 13,724 のまま、合計だけ 13,724 → 19,213(×1.4)

## 2026-08-25 極限スキル 2 枠(段階4b)

出典: wiki「Skill/極限」「Skill#f8e303fb(区分の凡例)」(取得 2026-08-25)。

1. **効果値は「基本 + スーパーリミット + ハイパーリミット Lv」の加算** / wiki の表の列がそのまま加算になっていて、合計が「最大値」列と一致する(スコープアイ 20+3+17=40、フルスロットル 75−3−17=55、ワイドフォーカス 4+2+14=20)。スーパーリミットは共通スキル「ハイパーアタック」、ハイパーリミットは「エクストリームアタック」が極限状態で変化したもの
2. **`UltimateSkills` は `CommonSkills` にぶら下げる** / 効果を底上げする 2 スキルがどちらも共通スキルの極限形なので、別の補正源にすると Lv の置き場所が割れる。**ハイパーリミット Lv2 以降も同じオーグメントゲート**に従う
3. **スコープアイはカテゴリG** / `evaluate` は非クリティカル時に `{F×G}` ごと 1.0 にしているので、非クリの一撃には自動的に乗らない(2026-08-25 段階6 の決定どおり)
4. **フルスロットルの段数は「単体チャネリングスキル」にだけ乗せる** / 対象の特定が課題だった。wiki「Skill#f8e303fb」の区分の凡例に `続` = チャネリングスキルとあり、**区分に `続` を含み対象指定が `単体`** が条件。19 キャラ 515 行をパースして **7 件**(極・連撃 / 極・無双乱舞 / 極・旋風斬 / 極・狂猫 / 極・双龍撃 / 極・紅龍連撃 / 極・滅神乱舞。いずれも段数 10)。`Skill::single_target_channeling` フラグにし、gamedata は id の const 配列で持つ(303 件のリテラルに引数を足さずに済む)
5. **ワイドフォーカスは収録するが火力には効かせない** / スキル範囲は与ダメージ式に入らない。3 択の 1 つなので「選べるが火力は変わらない」と分かるほうが誠実。効果ラベルに「(火力には効きません)」と出す
6. **同じ極限スキルを 2 枠に入れられない** / wiki は「2 つまで習得可能」。UI は片方で選んだものをもう片方の選択肢から外し、保存時にも検証する


## 2026-08-25 中ディレイと DPS(段階5)

出典: wiki「計算式まとめ `#ActualDelay`」「ステータス(中ディレイ倍率A / 倍率B)」、各キャラの
スキル性能一覧「動作」列(取得 2026-08-25)。

1. **式は wiki `#ActualDelay` の記載を正とする** / `中ディレイ = 基本中ディレイ × (1 − 中ディレイ減少値) × (2 コンボ以上なら 0.5)`、下限 0.3s。goal に当初書いた「倍率A × 倍率B」は wiki の現行表記と違う。倍率B の下限 30% = 減少値の上限 70% で同じことを言っている
2. **コンボボーナスは倍率A なので上限 70% の対象外** / wiki が「コンボボーナスは対象外」と明記。実装でも `min(70%)` を掛けたあとに ×0.5 する
3. **基本中ディレイはスキル性能一覧の「動作」列** / 303 件すべて取り込み、`ACTUAL_DELAYS` の const 配列で持つ(`SINGLE_TARGET_CHANNELING` と同じ理由で `s(...)` の引数には足さない)。読み替えは 3 つ
   - `A/B`(イェフネンのパイク系)は条件付き([加速])なので**基本値の A** を採る
   - `(固定)`(ティチエル 極・ギガブレイズ / クロエ 極・メテオストライク)は**中ディレイ減少が効かない**。`Skill::actual_delay_fixed`
   - 秒として読めない 1 件(ティチエル 極・スパークリングカイト。wiki 表記が `0`)は `None`。中ディレイも DPS も出さない
4. **チャネリングスキルの「動作」はチャネリング全体の秒数** / 極・連撃は 10s。2025/10/29 から全チャネリングスキルに中ディレイ減少が適用されるので、そのまま基本中ディレイとして使える
5. **キャラのパッシブは新しい補正源にした** / `StatSources::actual_delay_skills`(8 件のカタログは gamedata)。ステ補正ではないのでバフカタログには入れられず、`ElementSources` と同じ「`StatSources` にぶら下げるキャラ紐付けの供給源」に揃えた。`StatSources` は JSON 列なので **DB マイグレーション不要**
6. **共通の供給源は `calculate_damage` が自分で集める** / フルスロットル(`CommonSkills::ultimate`)・カフスの RO(`RandomOptionTotals`)・シエナのオーラ(`Equipment`)はすべて `DamageInput` に既にある。カタログ参照が要るキャラパッシブだけを呼び出し側が解決して渡す
7. **DPS は 合計ダメージ / 中ディレイ** / wiki 自身が前後ディレイを「省略」としているので前隙・後隙は入れない。CI(コンボインターバル)も未収録 → **後述の「実測のスキル回数表」で置き換えた**
8. **シエナのオーラの「防御力増加」「中ディレイ減少」に入力欄を付けた** / 段階4a でドメインには入れたが UI が無く、値を設定できないままだった(注記も「未収録」のまま残っていた)


## 2026-08-25 最終能力値への上限適用(段階6)

出典: wiki「Quest/覚醒クエスト」の「各能力の上限値」「極限後の各値の増加効果」、「エタの意志」の
「エタの成長」(取得 2026-08-25。表は段階4(前 goal)で `gamedata::awakening_caps` に取込済み)。

1. **上限は `effective_stat` の引数にした** / ダメージ上限(`damage_cap`)と同じく「表は gamedata、
   domain は値を受け取るだけ」。`apply_pins` のような後掛けの関数にすると `StatTrace::effective` が
   一度上限を無視した値になり、トレースの読み方が二重になる
2. **捨てられた分は `StatTrace::capped_loss` に残す** / ダメージ上限・防御力上限と同じ出し方。
   計算タブの詳細トレースに「上限 / 上限で捨てた分」の 2 列、キャラ画面の最終能力値に「上限」バッジを出す
3. **調整の「固定(pin)」は上限のあとに適用する** / pin は「実測値で計算したい時」の例外操作
   (`StatAdjustment::pin` のコメント)。ゲームの上限で削ると実測値が入れられなくなるので、
   `apply_pins` は上限適用後の値を上書きする(= pin は上限に縛られない)
4. **能力値プレビューにも覚醒を渡す** / `preview_effective_stats` は覚醒を取っていなかった。
   上限は覚醒段階 + エタの意志 Lv で決まるので、Tauri コマンドと Workspace のプレビュー呼び出しに
   `awakening` を追加した(キャラ画面で覚醒段階を動かすと上限も即時に動く)


## 2026-08-25 クリティカル率(段階7)

出典: wiki「計算式まとめ `#CriticalChance`」「狩り場情報一覧」(取得 2026-08-25)。

1. **AGI と クリティカル被撃率が両方そろった敵でだけ出す** / 被撃率A は −250〜−930% と式の中で支配的で、
   欠けたまま計算すると必ず 100% に張り付く。狩り場情報一覧は `?` の行が多いので、
   `Enemy::agi` / `Enemy::critical_taken_rate` を両方 `Option` にして、両方 `Some` のときだけ出す
   (AGI だけ分かる敵はデータとしては持つが率は出さない)。**「出さない」と「0%」を混同させない**
2. **スキルの Cri値が wiki 未記載(`-` の 28 件)でも出さない** / §9 の式の項なので、無い状態で 0 とみなすと過小になる
3. **敵の値は `[仮]`** / wiki 側が「約」「推定値」「情報源からの推定」と書いているコミュニティ実測。
   `ENEMIES_SOURCE` の note に明記した
4. **クリティカル率増加は値が確定している 3 件だけ** / wiki の表は「バフ n」「極のルーン 20」
   「設計者の研究室 30」「致命打 100」の 4 行。`n` は値が決まらないので入れない。上限 +100% は wiki の記載どおり
5. **供給源は `StatSources::critical_rate` に置いた** / ペット会心と 3 件のトグルだけなので専用の型
   (`CriticalRateSources`)。中ディレイと同じく `StatSources` の JSON 列なので **DB マイグレーション不要**
6. **未収録の入力は中立値** / シエナのオーラのクリティカル確率(S18)・被撃率B(対人のみ)・
   最終クリティカル率増加(「エクリプス+50%?」と wiki 自身が不確か)は入れない。UI の注記で明示する


## 2026-08-25 中ディレイと クリティカル率の修正(ユーザー指摘)

1. **ミラの「極・スパート」はマスタリー【グッドフェイス】前提の 5% 固定にした** /
   wiki「ステータス」の中ディレイ倍率B の表にある `-25%/-15%/-5%/-0%` を**段階選択**と読んで
   4 択にしていたが、これは誤り。wiki「Skill/ミラ #Spurt」に
   「中ディレイ減少(25%から9.6秒掛けて0%まで減衰)」とあり、**時間で減衰する値**なので
   定常値として扱えない。マスタリー【グッドフェイス】が「移動速度増加がなくなる代わりに
   持続時間増加(5分)、中ディレイ低下率固定(5%)」で、**減衰値を置き換える**(加算ではない)。
   よって `mira_spurt`(−5% 固定)1 件にまとめ、別枠だった `mira_mastery_good_face` を削除した
2. **`ActualDelaySkillDef` を 1 値(`percent`)に単純化した** / 1 の結果 8 件すべてが −5% 固定になり、
   段階選択の器(`percents` + `ActualDelaySkillChoice::choice_index`)が不要になった。
   選択も `ActualDelaySkills { skill_ids: Vec<String> }` に置き換えた(投機的な抽象化を残さない)
3. **エクリプスより下の低レベル帯のクリティカルは考慮しない**(ユーザー決定)/
   狩り場情報一覧で敵AGI・クリティカル被撃率が `?` の行(リンゴの島・忘却の地下墓所・
   ベリネンルミ・プラバ等)は追いかけない。段階7 の「値がそろった敵でだけ出す」方針のままでよい


## 2026-08-25 DPS を実測のスキル回数表で出す

出典: ユーザー提供の計測表「260702 테섭 허수아비 기준」(テスト鯖の訓練用かかし基準)。

1. **DPS は `合計ダメージ × 60 秒あたりのスキル回数 ÷ 60`** / wiki `#ActualDelay` 自身が
   「中ディレイ減少値を特定の値まで上げると、スキルの発動頻度が急激に変化する場合がある
   (丸め処理や通信頻度、入力間隔の都合)」と書いていて、実際の回数は `60 / 中ディレイ` に
   ならない(実測は式より **3〜14% 少ない** = 詠唱・入力の overhead)。式より実測を正とする
2. **表は総中ディレイ減少 % に直して持つ** / 元表の行は「フルスロットル45 + カフスRO3 = 48% への
   積み増し %」表記。うちの計算は総減少値を持っているので、48% を足した総減少 %(48〜70)で持つ
3. **`보리스 26% (실적용22%)` が実装の裏取りになった** / 48 + 26 = 74% が**上限 70%**で頭打ち
   になった結果で、`ACTUAL_DELAY_REDUCTION_MAX = 0.70` と一致する。
   同じく `그외 캐릭터 16%` = シエナのオーラ 2% × 8 部位、`밀라・비호・이솔렛 21%` = + パッシブ 5% で、
   供給源のモデルもそのまま合う
4. **格子の内側は 2 次元の線形補間、外側は式にフォールバック**(ユーザー確定)/
   表は 5 列(0.8〜1.6s)しかなく、スキルカタログの「動作」は 0.1〜10s まである。
   どちらで出したかは画面に明示する(「実測表」/「式で算出」)
5. **コンボボーナス時と「(固定)」の中ディレイでは実測表を使わない** / 表はコンボ無し・
   減少が効くスキルの計測なので、条件が違うものに当てると外れる
6. **CI(コンボインターバル)は考慮しない**(ユーザー決定)/ コンボによる DPS はサポート外
7. **クリティカル率増加の「バフ(値 `n`)」は旧仕様なので不要**(ユーザー確定)。段階7 の
   「値が確定している 3 件だけ収録」のままでよい


## 2026-08-25 アンリーシュ(能力解放)

出典: wiki「Skill/共通」の能力解放系列、「ステータス」の能力値倍率B の表(取得 2026-08-25)。
S19 の未解決点だった「ステのどのレイヤーに乗るか」がこれで確定した。

1. **効き先は能力値倍率B** / `ステータス` の `能力値倍率B (初期値0% 下限-30%)` の表に
   「共通スキル / アンリーシュ / +1〜20%」がある。`Skill/共通` の
   「バフ等含むステータス値 × 強化倍率[%]」も `最終能力値 = 基本能力値 + [基本能力値 × 倍率B] + 最終固定値`
   の倍率B と一致する。既存の `StatModifiers::multiplier_b` にそのまま加算する
2. **Lv1〜10 = +1/+2/+3/+4/+5/+8/+11/+14/+17/+20%** / wiki の表どおり。
   ユーザーの実測(基本能力値 506・最終固定値 479 で 990/995/1000/1005/1010/1025/1040/1055/1071/1086)と
   **1 の位まで一致**した。Lv9 だけ差が +16 になるのは `[506 × 0.17] = [86.02] = 86` の切捨て。
   実測をそのまま再現するテストを `common_skill.rs` に置いた。
   **ユーザーの実環境の内訳でも裏取りできた**: AGI 1086 =
   `[(素ステ310 + 固定値150) × 倍率A 1.1] = 506` + `[506 × 20%] = 101` + 最終固定値 479。
   固定値 150 = ペット 真AGI強化Lv2 40 + ルーン 20 + モンスターカード 70 + 全ステ固定 +20、
   最終固定値 479 = 聖物 350 + クラウン 100 + シエナのオーラ全ステ 29(逆算値と 1 の位まで一致)
3. **Lv6 以降はレインフォース(Lv1〜5)が前提** / オーグメントとまったく同じ形なので
   `reinforce_gated_level_max()`(= `reinforce_level + 5`)にした。UI もオーグメントと同じく
   「前提スキルを下げたら従属 Lv も一緒に下げる」挙動にする(保存だけ失敗するのを避ける)
4. **2 枠・同じステは不可** / wiki「2つまで使用可能」。`能力解放 - XXX` はステごとに別スキルなので、
   同じステを 2 枠に入れる状態は存在しない。UI ももう一方で選んだステを選択肢から外す
5. **`CommonSkills` に置いた** / 共通スキルなので `StatSources` ではなくこちら。
   `apply_siena_stats` と同じく `apply_unleash` を `build_modifiers` の後に呼ぶ
   (`StatSources` からは共通スキルに手が届かないため)。`common_skills` は JSON 列なので
   **DB マイグレーション不要**


## 2026-08-25 キャラ画面プレビューの再計算漏れ(共通スキル)

**決定**: `Workspace.svelte` の即時プレビューの `$effect` で `commonSkills` を
`JSON.parse(JSON.stringify(...))` の deep copy にする(装備・`statSources` と同じ形)。

**理由**: `{ ...draft.commonSkills }` の浅いコピーだと、`$effect` が読むのはトップレベルの
プロパティだけなので、`unleash[0].level` のような**ネストした値の変更で再計算が走らない**。
アンリーシュを追加して初めて表面化したが、極限スキル(`ultimate.hyper_limit_level` 等)にも
同じ穴があった(こちらは能力値に効かないので気付けなかった)。

**確認方法**: 検証ボリスで アンリーシュ DEX Lv6(+8%)を選ぶと最終能力値 DEX 199 → 212
(`[基本能力値 × 8%]`。最終能力値に掛けているのではないことも確認)。


## 2026-08-25 モンスターカード(カード装着)

**決定**: `StatSources::monster_cards`(ステごと 0〜70)を新設し、**固定値層**に足す。

**理由**: wiki「ステータス」の固定値増加の表に「モンスターカード / カード装着 / **+0〜70** /
装着カードのステータス」がある。補正源の一覧では「これから」のまま残っていたが、
ユーザーの実環境の照合(AGI 1086)で **+70 が実際に乗っている**ことが分かり、
入れないと最終能力値が 70 ぶん足りなくなる。

- **固定値層**(倍率A の前)なので、テイルズウィーバーのエネルギー等の倍率A の影響を受ける。
  クラウン・神鳥の聖物(最終固定値層)とは別枠
- ルーン・クラウンと同じ「ステごとの手入力」の形。`StatSources` は JSON 列なので
  **DB マイグレーション不要**
- 属性値の供給源にも「モンスターカード +30」があるが、あちらは `ElementSources` で別物

**確認方法**: ユーザーの実環境の内訳が残差ゼロで再現できるようになった。
`固定値層 150 = ペット真AGI強化Lv2 40 + ルーン 20 + モンスターカード 70 + クラブSエフェクト 20`
→ `基本能力値 [(310 + 150) × 1.1] = 506` → `最終 506 + [506 × 20%] + 最終固定値 479 = 1086`。

**クラブ効果は +7 固定のままにする**(ユーザー決定 2026-08-25): wiki の表記は「+1〜7」だが、
装備強化・ランダムオプションと同じ「上書きが無ければ上限」の方針に揃う。
**+20 は別バフの「クラブSエフェクト」**で、こちらも収録済み。排他枠が空なので両方同時に掛けられる
(wiki の固定値増加の表でも別行で、重複不可の注記なし)。


## 2026-08-25 シエナのオーラの残りの追加オプション(S18)

出典: wiki「装備システム/シエナのオーラ」の追加オプション一覧、「計算式まとめ `#CriticalChance`」、
「ステータス」の [C] 対象の防御力(取得 2026-08-25)。

1. **「クリティカル確率」は収録する** / 追加オプション一覧の備考が
   「[[クリティカル率が割合増加>計算式まとめ#CriticalChance]]」で、`#CriticalChance` の式では
   **AGI 由来の項に掛かる乗数**(ペット会心と同じ位置)。段階7 でクリティカル率を実装したので
   効き先が確定した。1 部位 0〜10%(最大帯 8〜10%)で、`Equipment::siena_critical_rate` に集計し
   `× (1 + Σ%)` で掛ける
   - wiki は「シエナのオーラ : 同一名称の効果同士で加算されるかどうかは要検証」としているが、
     攻撃力増加・防御力増加・中ディレイ減少と同じく**部位ぶん加算**する `[仮]`
2. **「防御無視攻撃確率」は収録しない** / wiki「ステータス」の [C] 対象の防御力の表では
   `x%/-15%` = **x% の確率で防御力 15% 無視**。確率発動なので、エレメンタルブレイク(S21)と
   同じ理由で入れない。さらに wiki 自身が「他の強打効果と同時に装着すると防御力無視量が
   加算される不具合(?)あり」「複数装着しても効果がさらに増幅したり確率が変動したりはしない」と
   書いていて、期待値ですら確定しない
3. **HP/MP/SP 増加は対象外** / 与ダメージ式にも防御側の計算にも入らない

これで S18 は「クリティカル確率 = 実装済み / 防御無視攻撃確率 = 収録しない」で閉じ。

## 2026-08-25 デザインシステムの決定 6 件(docs/claude/goals/2026-08-25-design-conformance.md)

デザインシステム(docs/design-system.html)を整備する過程で出た、**規格だけでは答えが出ない論点**。すべてユーザー確定。規格側は同 §14「これから作る画面の決定」に置いた(実装したら消して該当節へ移す)。

1. **ダメージは「鎖」で出す。1 発と DPS を並列に置かない** / 1 発は**ゲート**(防御を抜けるか・目安を超えるか = 閾値判定)、DPS は**レート**(連続量)で、同じ種類の量ではない。ゲートを通らなければレートに意味が無い(防御を抜けていなければ、どれだけ速く振っても 0)ので、軸の切り替えではなく因果の順に繋ぐ: `1発 → ×段数 → 合計 → ÷中ディレイ → DPS`。既存の「なぜこの数字？」の 1 行フローと同じ文法で、**新しい軸ではなく既存の鎖の延長**。ヒーローは 1 発のままなので `--t-result` の 44px は増えず、到達判定も per-hit に固定されたまま巻き戻しが起きない
2. **DPS に到達判定(バッジ)を付けない** / 「何秒までなら合格」の基準がゲーム側に存在しないので、判定を作ると嘘になる。DPS は比較のためだけの数字にし、**判定を持つのは per-hit だけ**という非対称をはっきりさせる(状態 6 系統の意味を守るため)
3. **主役は画面ごとに分ける。1 画面で軸を切り替えない** / ホーム・計算タブ = ゲート主役(行けるか)、強化提案タブ = レート主役(次に何を伸ばすと効くか)。強化提案が DPS を要るのは、フルスロットル(段数 +3・中ディレイ短縮)が **per-hit を 1 も変えずに実効火力を上げる**ため。ゲートだけ見ていると候補の順位が付かない
4. **効いていない分の棚卸しは 1 箇所に集める** / ロスは カテゴリ上限 / ダメージ上限 / 防御力上限 / 能力値上限 / 中ディレイ下限 の 5 階層になる。斜線の記号は各所に残したまま、**棚卸しだけを「なぜこの数字？」の中の 1 セクションに集約**する。5 箇所を回らないと総ロスが分からない状態は「効いていない量を見せる」という価値を薄める。新画面は作らない
5. **補正源リストは操作頻度で 3 群に分ける(毎回変える / 育ったら変える / 一度決めたら触らない)** / 10 行が同じ行の重さで並び、オーグメント Lv と常用バフが同格になっていた。行数は減らさず(要素が増えても入口は変わらない)、重さだけ変える。固定群は既定で畳んで「N 件・設定済み」の 1 行に要約する
6. **相手依存の値は「持っている値」と「その相手への効き」に割る** / キャラ側(補正源)には相手がいなくても決まる値だけを置き、対象を選んで初めて出る値は計算タブ側に置く。**先例は属性**(キャラ側に属性値、計算側に敵属性値との差 = カテゴリ I)で、この形が正解だったので規約にする。次に守る対象は**クリティカル率**(対象の AGI・クリティカル被撃率は敵側の値なのでキャラ画面に置かない)。破るとキャラ画面に必ず「—」になる欄ができ、それは欠損ではなく置き場所の誤り
7. **試し変更は同時 3 件までに制限する** / スキル・極限スキルまで試し対象が広がるとチップ列が読めなくなる。ここで「試しセットに名前を付けて保存」に逃げると **ラベンダー = 保存されない の意味が壊れる**ので、色の意味を守るために機能側へ制約を置く。上限に達したことは色ではなく文言で伝える(ラベンダーに 2 つ目の意味を持たせない)
8. **欠けの表現を行内から行頭へ移す(収録度バッジ)** / 「値のある敵だけ出す」は正しいが、30 行の一覧で破線が増えると画面が壊れて見える。**欠けの表現には密度の閾値がある**というのがここで分かったこと。行頭に「この敵はどこまで分かっているか」を 1 つ置き、行内は静かにする。完全な行にはバッジを出さない(全行に並ぶと装飾になる)

### 訂正

当初「属性が未実装なので所属を先に決める」としたが、**属性は実装済み**(wiki-gaps 段階3 + レビュー反映。補正源に属性ペイン、計算側でカテゴリ I)。決定 6 はその実装を一般則に昇格させたもので、新規の設計判断ではない。

## 2026-08-25 デザインシステム適合ループ 1 周目(docs/claude/goals/2026-08-25-design-conformance.md)

形式は goal 側の記録が正。ここには**規格そのものを動かした判断**だけを残す。

### 機械監査

- **`tools/design-audit/run.py` を置き、CI には入れない** / 検出は候補どまりで、違反かどうかは「実装と規格のどちらが正しいか」の判断が要る。自動で落とすと規格の穴を実装に押し付ける / `python tools/design-audit/run.py`(9 ルール)。1 周目で 188 → 0

### 色の持ち方(§03 / §15)

- **状態の 6 系統は `app.css` の `--state-*` が持ち、参照は `src/ui/states.ts` の `STATE` / `badgeStyle` に一本化する** / それまで CalcPage と HomePage が同じ 6 系統を hex 配列でコピーしていて、片方だけ増えて 7 要素 / 10 要素に割れていた。画面側は「ラベンダーの言葉 → どの系統か」だけを持つ / 機械監査 R1 / R8 が 0
- **「内訳の色」を状態の 6 系統から独立させる(`--flow-base` / `--flow-1`〜`--flow-7`)** / 積み上げ帯・流れ帯の区間色は達成度ではなく識別で、6 系統に寄せると「緑 = 足りている」の意味が壊れる。規格に居場所が無かったので §03 に節を足した。逆にこの並びをバッジや到達バーに使うのも禁止
- **予約色の金・敵の赤茶・緑の明にトークンを与える(`--gold` / `--mob` / `--good-soft`)、文字色 6 段を全段トークン化する(`--fg-head` / `--fg-sub` / `--fg-off`)** / 規格が色だけ並べていて、どの変数がそれかは実装を読まないと分からなかった。§03 に対応するトークン名を併記した
- **`--good-bg` / `--good-border` / `--danger-bg` / `--danger-border` を削除する** / 用途が `COST_COLORS` 一つだけで、§03 が「コストの色は 6 系統をそのまま流用する」と言っている以上、別名を持つ理由が無い(後方互換を残さない)
- **上限到達の「満」は金** / 実装は赤茶(`--danger`)で、赤茶は §03 で「敵と危険・届かない」。上限に届いたことは危険ではない。ただし**面**を金に塗るか紫に塗るかは §03 と §07 が食い違っており、§14 に保留として登録した

### 入力(§04 / §07)

- **上限は値の隣に常設する(`StatInput` の `capGauge` を調整ペインにも渡す)** / MAX ボタンだけでは上限が読めず、§07 の「範囲外を打てなくするのではなく、範囲を見せる」を満たしていなかった
- **枠や地を持つ操作部品に角丸 0 を許さない** / §04 は「規格シート 2a の 0〜4px は硬すぎ」として採らなかった方向。`select` / 数値欄 / MAX / チェックボックスが 0 のままだった。機械監査に R9 として入れた

### 欠けの見せ方(§00 / §09)

- **「足りない分をどう埋める?」は候補 0 件でもパネルを残す** / カラム帯に常設のリンクがあるのにパネルごと消えると、指す先が無くなる(§09 要素が増えても入口は変わらない)。「変えられる場所が無い」のか「候補はあるが超えない」のかを書き分ける(§00 欠けを正常な状態として見せる)

### 保留(design-system.html §14「判断を保留しているもの」)

A. 数値入力のスライダーが §07 の 5 形態のどれでもない / B. オン・オフの選択がチェックボックスとチップに割れている /
C. 上限到達を金で言うか紫で言うか(文書内の食い違い)。いずれも **v4 の実物を見ないと決められない**ので、決めるまで違反として数えない。

## 2026-08-25 デザインシステム適合ループ 2 周目(docs/claude/goals/2026-08-25-design-conformance.md)

1 周目は機械監査で拾える下層(色・角丸・書体)に寄っていた。この文書の重心は §00 の 15 項目で、
「規格(色・角丸・書体)は結論であって理由ではない」と冒頭に書いてある。2 周目はそこから見直した。

### 動きと変化(§10)

- **§10 は実質未実装だったので、6 つの型を app.css に置いて配線した** / `scale()` 0 件、`@keyframes` 0 件、`prefers-reduced-motion` 0 件だった。§00-7「破ると『押したのに何か起きたのか分からない』。試し変更が主要な体験のこの道具では機能そのものが伝わらなくなる」に真正面から反していた / 実機で 6 型すべての発火と reduced-motion(0.28s → 1e-05s)を確認
- **数値の増減の色は animation で当て、終了時に元へ戻す** / `fill-mode` を使わなければ後始末が要らない。色を残すと状態色(§03)と意味が混ざる
- **`bump` は値ではなく getter を受け取る** / Svelte 5 の action は引数が変わっても再実行されない(`update` が廃止)。中の `$effect` が getter を読む形にする

### 空間と視点(§09)

- **装備のドリルダウンを 3 ペインにする(補正源 / 部位一覧 / 部位詳細)** / 部位行を押すと一覧が消えてアイテム選択に入れ替わっていた。§09 規則 2 は「掘るたびに右へペインが増える。前の階層は残るので、戻るのに『戻る』を押す必要がない」 / 押した行の座標が変わらないことを実機で測って確認(x/y 不変、幅だけ 707→232)
- **補正源の並びはプレイヤーが決める(お気に入り + ドラッグ&ドロップ)** / 最初は操作頻度で 3 群に固定したが、**どれをよく触るかは人によって違う**。§09 規則 5「並びは、ユーザーが頼まない限り変わらない」の裏返しで、頼まれたら変わってよい。設定画面は作らず、リストの上で直接動かす。★ はホームタブのコンテンツと同じ操作なので覚えることが増えない
- **群を上げ下げする ▲▼ は採らない** / 動かしたい先が見えているのに 1 段ずつしか動かせず、押すたびに対象が飛ぶ。並べ替えの操作としては最悪(ユーザー指摘)

### §14 の 6 決定がすべて着地

鎖 / 効いていない分の棚卸し / 補正源の並び / 相手依存の値 / 欠けの表現 / 試し変更の上限。
§15 のルールに従い §14 から消し、§02・§03・§06・§09 へ実物として移した。
§14 は「これから作る画面の決定」から「まだ決めていないこと」になった。

- **判定はゲートの位置にだけ置き、レートには付けない** / 「何秒までなら合格」の基準がゲーム側に存在しないので、DPS にバッジを付けたら嘘になる
- **上限で捨てた分の棚卸しは「なぜこの数字?」の中に 1 つだけ** / 5 階層(カテゴリ / ダメージ / 防御力 / 能力値 / 中ディレイ)を回らせない。中ディレイ下限はこれまで「※下限 0.3s」の警告どまりで量が出ていなかった
- **収録度バッジは完全に分かっている行には出さない** / 全行に並ぶとバッジが装飾になる。欠けの表現には密度の閾値がある

### 保留(design-system.html §14)

A. スライダーは §07 の 5 形態のどれか / B. オン・オフがチェックボックスとチップに割れている /
C. 上限到達を金で言うか紫で言うか / D. 押した候補が消えるのをどう見せるか(§09 規則 5 は
「消えることを動きで見せる」を求めるが、§10 に「行が消える」型が無い)。
いずれも **v4 の実物を見ないと決められない**ので、決めるまで違反として数えない。

## 2026-08-25 デザインシステム適合ループ 3〜11 周目(完了)

11 周まわして終了条件を達成した。3〜9 周目で見つけた違反はすべて
**「規格に具体的に書いてあるのに実装が見ていなかった」**もので、周を重ねるごとに
規格の読み方が深くなった。10・11 周目は新規違反 0。

### 座標を測る

- **§09 規則 1・4 は目視ではなく座標で測る** / 「押した場所は動かない」「桁が増えても隣が動かない」は
  見た目では分からない。Playwright で値を変えて `getBoundingClientRect()` を比べる /
  実際にこれで「最終能力値の列が桁で 12px ずれる」を見つけた
- **見切れている行を押すと、ブラウザがフォーカスを視界に入れてスクロールする** / それは規則 1 の
  違反ではない(むしろ規則 5 に沿う)。**完全に見えている要素で測る**
- **値が上限に張り付いていると操作しても数字が動かない** / §10 の検証にならないので、
  上限に当たっていない値で測る。同時に、これは「押したのに何も起きない」という §00-7 の
  破れそのものでもあり、決定 2(効いていない分の棚卸し)が要る理由になった

### 規格の読み方

- **§08「部品」と §11・§12「組み上がり」は寸法まで書いてある実物の節** / 新しい部品を作るときも
  既存を直すときも、まずここと突き合わせる。6 周目に自作の見た目で段階選択を書いてしまい、
  7 周目にスプリッターの当たり判定が 6px(規約は 9px)なのを見つけた
- **§00 の 15 項目が文書の重心** / 「規格(色・角丸・書体)は結論であって理由ではない。理由はここにある」と
  冒頭に書いてある。1 周目は機械監査で拾える下層に寄っていて、§10 が実質未実装なのを見落とした

### 入力(§07 / §08)

- **素の `<select>` とチェックボックスは 5 形態のどれでもない** / 形態 2 は「段階そのものを見せる」で
  モックはセグメント、形態 3 は「値はチップの中に書く」。順序があって並べられる有限選択は
  `StepSelect.svelte` に寄せた
- **フィールドは「表示が既定・編集は例外」** / 初期値は常に埋まっているので、ふだんは読み取り表示。
  入力欄は自動値を上書きする例外操作。スライダーが常時出なくなったぶん 1 画面の情報量も増えた
- **範囲は常設する** / `StatInput` は 22 箇所で使っているのに範囲を出していたのは 3 箇所だった。
  既定 true にした

### 機械監査に足したルール

R9(枠のある操作部品に border-radius が無い)と R10(font-size が実寸スケールの外)は、
実機で見つけたものをスクリプトに落としたもの。**実機で 1 回見つけたら機械監査に入れる**と、
次の周から人が見なくてよくなる。

### 残した保留(§14 A〜F)

スライダーの位置づけ / チップとチェックボックス / 上限到達の色 / 消える候補の見せ方 /
ドロップダウンの居場所 / 装備ペインの下の帯。**6 件とも v4 の実物を見れば決まる**。


## 2026-08-26 コアセット効果は進化段階ごとに成立して合算する

**セットは「同じ進化段階の強化 4 コア」で組む。**進化段階ごとに成立した分を**合算**する
(ユーザー確認 2026-08-26)。

- 進化3強化4 ×3 + 進化4強化4 ×3 → 進化3 の 3〜5 セット +1% と 進化4 の 3〜5 セット +2% で **+3%**
- 進化4強化4 ×6 → 進化4 の 6 セット **+5%**(3 セット分は重ねて数えない)
- 上位のコアが下位のセットを埋めることはない(進化4 ×2 + 進化2 ×4 → 進化2 の 3〜5 セットだけ)

wiki「テシスコア/コアセット効果」の表が **3〜5 セット効果 / 6 セット効果**と個数で分かれているのが
この形の根拠。**それまでの実装は「成立する最も高い進化段階を 1 つだけ採る」で、混在時に過小評価していた**
(3-4 ×3 + 4-4 ×3 を +2% と出していた)。
...
## 2026-08-26 武器のランダムOP は「条件を満たしている前提」で計算に入れる

武器には**火力に効く OP が同時に 3 つ載る**(ユーザー確認 2026-08-26)。例:

- 真・一般ボスモンスター攻撃時、追加ダメージ(カテゴリー**1**)
- 魔法攻撃が的中した場合、確率で追加ダメージ(カテゴリー**11**。wiki の表は 1 に置いているが
  **実際は 11**。ユーザー確認)
- 強化の石を 1 個消耗する代わりに追加ダメージ(カテゴリー**16**)

カテゴリーが違うので「同じカテゴリーは 1 部位に 1 つ」の制約とも矛盾しない。

**発動条件(ボス限定・確率・石の消費)は満たしている前提で、常に効くものとして計算に入れる。**
確率つきは期待値に割らず、効いたときの値をそのまま使う。条件はカタログの `note` に残して画面に出す。

- 効き先: `RandomOptionEffect::AddedDamageRate`(§5「新-割合」)。合計ダメージに乗る
- 供給源はシャープネスビジョンと合算する(`damage.rs` の `added_rate`)
- これまでは 武器の OP をすべて `RecordOnly` にしていたので、**武器の OP がダメージに
  1 も効いていなかった**

**種別(新-割合)の根拠**は wiki「ステータス」`#追加ダメージ(割合)関連` の一覧。ここに
**武器のランダムOP が 20 件以上、脚のランダムOP が 2 件、すべて「追加ダメージ(新-割合)」の表に
列挙されている**(一般ボス +2〜21% / 物理・魔法命中時 +10〜15% / 強化の石 +18〜48% など)。
推定ではなく wiki の分類そのもの。脚の 2 件(カテゴリー17)もこの機会に収録した。


## 2026-08-26 シエナのオーラは「スロットを 1 個ずつ選んで積む」

2026-08-24 の決定 3(**入力粒度は部位ごとの合計値。能力値スロット 1 個ずつは持たない**)を
**取り消す**。ユーザー決定: 「段階は不要。入力数 = 解放数になるから、補正値は加算式なので
選択してポンポン積み上げた方がいい」。

当時「個別に持たせても計算結果は合計と同じで入力コストだけ増える」と判断したが、**逆だった**。
合計値モデルは画面に次の 3 つを持ち込んでいた。

1. **段階を別に入力させる。**段階は解放スロット数と定義上等しいのに、入力欄が 2 つある
   (段階 11 個のボタン + 値の欄)。しかも段階 1 でも 9 値 + 追加オプション 5 欄が全部出る
2. **wiki の内訳をユーザーに計算させる。**「物理複合 5 = 突き 3 + 斬り 2 に分けて入れてください」
3. **その部位に出ない能力値まで欄に出す。**wiki の一覧は武器/盾 6 種・その他の部位 12 種なのに、
   装備補正 9 値と STAB〜AGI 7 値をそのまま出していた

1. **`SienaAura` は `slots: Vec<SienaSlot>` + `extras: Vec<SienaExtraSlot>` だけを持つ。
   増幅段階は `slots.len()`、追加オプションの枠数は段階から出す(3/7/10 で 1/2/3)** /
   wiki「発現・増幅」の表が「段階ごとに能力値スロットが 1 個解放」と書いている。
   段階はスロット数の言い換えなので、別に持つと必ず食い違う /
   `crates/domain/src/siena.rs`、`SienaAura::stage`/`extra_capacity`
2. **選べる種類と値域は wiki の一覧表そのままを domain のカタログにし、コマンドで画面へ渡す** /
   中身はランダムだが**種類と値域は静的データ**なので、ux-guidelines 原則 1(wiki から取れる値は
   入力させない)がここまでは適用できる。ランダムOP の `RandomOptionDef` と同じ依存方向 /
   `siena_catalog()`、`list_siena_kinds` コマンド、`app.siena`
3. **複合(物理複合・魔法斬り)は domain が分解する** / wiki「物理複合攻撃力5 の場合、突き 3・
   斬り 2 増加する」。大きいほうが先(`(value + 1) / 2`)/ `SienaValueKind::apply_to_values`
4. **与ダメージ式に入らない種類(物理/魔法ダメージ耐性・クリティカル被撃率減少・命中率・回避率・
   防御無視攻撃確率・HP/MP/SP)も選べるようにし、グレーの「記録するだけ」枠にする** /
   段階をスロット数から出す以上、**実際に出ているスロットを全部記録できないと段階が合わない**。
   ランダムOP の記録のみ枠と同じ扱い / `SienaValueKind::is_modeled`
5. **既存の入力値は移行せず捨てる**(ユーザー決定)/ 装備は JSON 列なので、旧形式の
   `stage`/`values`/`stats` は未知フィールドとして無視され、オーラは未発現から入れ直しになる。
   後方互換を保たない原則どおり / `crates/storage` に移行コードは足していない
6. **「クリティカル確率は未収録」という UI の注記は誤りだった**(`siena_critical_rate` として
   AGI 由来の項に乗算で入っている)。文言を直した / `SourcePane` のクリティカル率ペイン

画面側は docs/claude/goals/2026-08-25-design-conformance.md の 18 周目に記録した
(§09 規則 1 違反 2 件を含む 5 件を修正)。


## 2026-08-26 称号の「ダメージ n% 増加」を取り込む(称号/event)

ユーザー指定(2026-08-26): 黒虎のオーラ / 辛卯黒兎 / 青龍灼然 / 蒼蛇の幽影 / 緋馬の怪火 が必須。
5 件とも wiki「**称号/event**」にあり、カタログは normal / special しか見ていなかったので全部未収録だった。

調べたら**未収録だったのは称号そのものより効果のほう**で、この系統の価値は補正値ではなく
**無条件の「ダメージ n% 増加」**にある(緋馬の怪火は単一ステ +90 + ダメージ +20%)。
`TitleDef` は `values` しか持っていなかったので、選んでも 20% が計算に入らなかった。

1. **効き先は カテゴリX(攻撃ダメージ)の X3 基本発動** / wiki「ステータス」`#z4747f51` の
   `[X3]攻撃ダメージ(基本発動) (上限:+80%)` の表に
   `称号 → [[ダメージ増加>称号#c3803146]] +x%` が載っている。リンク先アンカー `#c3803146` は
   称号/event の「明鏡止水」= 課金箱シリーズそのもの。推定ではなく wiki の分類 /
   `TitleDef::attack_damage_percent`、`title_attack_damage_rate`、`damage.rs` の X 集計
2. **`TitleKind` に `Event` を足す** / wiki のページ分けが normal / special / event の 3 つ。
   **無条件のダメージ増加を持つのは event だけ**で、normal / special の「〜関連マップで追加ダメージ +20%」は
   発動条件付きなので従来どおり `note` 止まり。この違いを型で見えるようにする
3. **event の収録基準は「無条件のダメージ増加を持つこと」**(48 件)。補正値 9 種の合計が 0 でも入れる
   (「専門家 - ダメージ」)/ 合計 15 以上で切ると event だけで 200 件超になり、しかも
   normal / special の同水準と入れ替わるだけで選択の役に立たない。**選ぶ理由になっている効果**で切る /
   `crates/gamedata/src/titles.rs`
4. **カタログの並びを「ダメージ増加 → 与ダメージに効く 1 値の大きさ → その 4 値の合計 → 9 値の合計」に変える** /
   従来は 9 値の合計順だけだった。**装備攻撃力に入るのは突き / 斬り / 魔攻 / 魔防の 4 値だけ**で、
   しかもキャラ 1 人が使うのは**そのうち 1 つ**(スキルの依存種別)。9 値に薄く散らした称号を
   合計で上に出すと、実際に強い称号が埋もれる(ユーザー指摘 2026-08-26: 課金箱シリーズが
   名誉の証に押し下げられていた)。例: 緋馬の怪火 - 突き は 9 値合計 90 だが、突き依存のキャラには
   名誉の証(トーデン兄妹。9 値合計 205)より効く。`title_catalog()` の末尾で並べ替える
   (ファイル上の並びは wiki の節ごとのままにして、追記のたびに手で挿し込まない)
5. **同じ称号の依存違い(「緋馬の怪火 - 突き / 斬り / 魔法攻撃 / 魔法防御」)は一覧で 1 行にまとめ、
   変種はチップで選ぶ**(ユーザー指摘 2026-08-26)/ 4〜6 個なので §07 形態 2 に収まる。
   120 件が 70 行になり、マーキュリアル洞窟のボス別称号(1 ボス 6 変種)も同じ形で畳める /
   `SourcePane` の `titleGroups`
6. **「名誉の証(Supporter)」は未収録** / wiki の 1 行が a〜d の 4 種をまとめており、しかも中身が
   装備補正ではなく**ステ加算**(STAB/HACK/INT/MR +60)。この器では表せない


## 2026-08-27 設計者の研究室は段階制(オン/オフをやめる)

ユーザー指摘(2026-08-27): 設計者の研究室のバフは段階的に入力できるようにしたい。
実装はチェックボックス 1 個(+30%「最大レベル時」)で、**1〜9 段階の人が自分の値を入れられなかった**。

wiki「設計者の研究室」の永続バフの表に **B グループ「クリティカル率増加」= 1 段階あたり +3**、
概要に「A・B グループは最大 10 段階まで研究可能」とある。+30 は 10 段階時の値だった。

1. **`CriticalRateSources::architect_lab: bool` を `architect_lab_stage: u8`(0..=10)に置き換える** /
   値は `段階 × 3`。上限 +30 は段階から出るので定数で持たない。
   `CriticalRateSourceId::value()` は `max_value()` に改名し、実際に乗る値は
   `CriticalRateSources::value_of` が返す(設計者の研究室だけ段階制で、ほかはオン/オフ)/
   `crates/domain/src/critical_rate.rs`
2. **値域の検証を `CriticalRateSources::validate` として持ち、`StatSources::validate` から呼ぶ** /
   ほかの供給源(ルーン・クラウン・聖物)と同じ位置づけ。`StatSourceError` に透過で合流させる
3. **UI は §07 形態 2(段階選択)。0〜10 を 11 セルで並べる** / チェックボックスの列は割らずに
   その下に 1 段だけ置く。ラベルは数字だけにそろえ、「0 = 未研究」は欄の説明に書く
   (「未研究」を 1 セルに入れるとそこだけ幅が要る)/ `SourcePane` の `.lab-field`
4. **旧データは移行しない** / 装備・補正源は JSON 列なので `architect_lab: true` は未知フィールドとして
   無視され、0 段階から入れ直しになる。後方互換を保たない原則どおり

なお wiki の同じ表に **B グループ「命中率増加」= 1 段階 +3** もあるが、命中の配線は別件なので今回は入れない。


## 2026-08-27 マスタリーは「段ごとに 1 つ」の独立モデルにする

ユーザー指摘(2026-08-27): キャラクタースキルのマスタリーは切り替えできるようにしたい。
調べたら切り替え以前に**モデルが無かった**。wiki(Skill/ボリス、取得 2026-08-27)のスキル表は
`P (M1)`〜`(M4)` の 4 段で、**どの段も 3 択・1 つだけ取る**。

| 段 | 選択肢 | 効き先 |
|---|---|---|
| M1 | 一閃 / 斬撃 / 一撃 | 中ディレイ −5% / 攻撃ダメージ +2% / 防御力無視(確率) |
| M2 | 抗争の生存者 / 必滅者 / シルバースカル優勝者 | 被ダメ −3% / 攻撃ダメージ +3% / HACK・DEF +10% |
| M3 | インデュア / スイッチング / 騎士道 | すべて防御側・条件付き |
| M4 | 剣を扱う技術 / 冬を降らせる子 / 冷酷で絶対的な審判者 | HACK・DEF +10% / 条件付き / 攻撃ダメージ +2% |

**同じ段の選択肢は効き先がばらばら**なので、既存のカタログに散らすと段の排他が
カタログをまたいで表せない。実際そうなっていた: 一閃だけが `actual_delay` のカタログ、
シルバースカル優勝者だけが `buffs` のカタログにあり、**M2 の残り 2 択と M4 は丸ごと未収録**。
「シルバースカル優勝者を取っていない」= 他の 2 つのどちらか、という状態が入力できなかった。

1. **`crates/domain/src/mastery.rs` を新設し、段ごと 1 つの型にする** /
   `MasteryEffect { StatRate | ActualDelay | AttackDamage | RecordOnly }` が効き先を持ち、
   `Masteries::validate` が「段の重複」「他キャラのマスタリー」を弾く。
   効き先ごとの合流は `actual_delay_reduction` / `attack_damage_rate` / `stat_rates` が返す
2. **計算に入らない選択肢も `RecordOnly` として収録する** / 段は 1 つしか取れないので、
   計算に入らない選択肢を落とすと**段の状態そのものが表せない**(ランダムOP のグレー枠と同じ理由)。
   ボリスは 12 件中 6 件が RecordOnly
3. **「攻撃ダメージ +n%」はカテゴリX へ入れる**(ユーザー決定 2026-08-27)/
   称号・ランダムOP で X の配線がもう通っている。ボリスは M1 斬撃 +2% / M2 必滅者 +3% /
   M4 冷酷で絶対的な審判者 +2% の 3 件 / `DamageInput::mastery_attack_damage_rate`
4. **入力の場所は補正源「キャラスキル」の中**(ユーザー決定 2026-08-27)。専用の補正源は作らない /
   段ごとに「未取得 + 3 択」のチップ 1 段(§07 形態 2)。中ディレイペインからは
   「ほかの補正源から入る分」でキャラスキルへ飛べる
5. **`actual_delay` のカタログから【一閃】、`buffs` から【シルバースカル優勝者】を削除する** /
   同じものを 2 箇所に置かない。中ディレイのカタログは**キャラ固有のパッシブだけ**になった(8 → 7 件)
6. **消えたバフ id は起動時の移行で落とす**(`migrate_removed_buffs`)/ カタログに無い id が
   保存済みデータに残ると `build_modifiers` が `UnknownBuff` で弾き、**そのキャラの計算が
   まるごと止まる**(実機で「未知のバフです: boris_silver_skull」が出た)。
   カタログを入れ替えるたびに `REMOVED_BUFF_IDS` へ 1 行足す

7. **一覧は「アイコン + 名前 + 効果」のカードを 3 列で並べる**(ユーザー指摘 2026-08-27)/
   どの段も 3 択なので列を固定すると目が横に動かない(§00 01)。効果をカードに出さないと
   「どれを選ぶか」を別の資料で調べることになる(§00 05)。長い効果は 2 行で打ち切り、
   全文は `title` に出す(打ち切らないと騎士道・冬を降らせる子だけ段が高くなる)
8. **ゲームに「未取得」という選択肢は無いので出さない**(同指摘)。外すときは選んでいるものを
   もう一度押す。ドメイン側は「まだ選んでいない」を表せる(空の `picked`)ままにする —
   ユーザーがどれを取っているか分からないのに勝手に 1 つ入れない
9. **アイコンは wiki の添付画像を同梱する** / `?plugin=ref&page=<EUC-JP エンコードのページ名>&src=<ファイル名>`
   で取れる。**ページごとの添付**なので、キャラが違えば `Mastary2_1.png` でも別の絵になる。
   `Icon.svelte` に `mastery` 系統(`assets/icons/masteries/<id>.png`・枠はスキルと同じ)を足し、
   id から機械的に解決する(手動のマッピング表は作らない)

### 全キャラ分の取り込み(2026-08-27)

wiki の各 Skill ページの「**マスタリースキル** `[#MasteryList]`」表から **18 キャラ 243 件**を収録した。
インラインのスキル表(`P&br;(M2)` の行)ではなくこの節を読む — 表が
`|No.|開放Lv|モード|スキル名|解説|` で機械的に読めるうえ、解説が長い形で入っている。

- **段の数はキャラによって違う**(3〜5 段。リーチェだけ 15 段)。`MASTERY_TIERS` の固定 4 段は捨て、
  段はカタログの `tier` から出す
- **アナイスは未収録** — wiki の Skill/アナイス にマスタリーの節が無い(コメントアウトされている)
- **id は wiki の表の位置そのまま**(`<キャラ>_m<段>_<1..3>`)。243 件の日本語名をローマ字にすると
  読みを確かめられず、まちがえても気づけない。表の No. 列が段、モード列
  (Primary / Secondary / Tertiary。ベンヤだけ DEATH / BLOOD / DEFENCE)が 1..3
- **計算に入れるのは「無条件で言い切っているもの」だけ**(243 件中 **11 件**)。
  「〈スノーガード〉使用中」「スキルを選択」「確率で」のような条件付きは条件を持っていないので
  `RecordOnly`。**誤って計算に入れるほうが、入れないより害が大きい**(数字が黙ってずれる)。
  この 11 件はテストで id を列挙して固定してある(黙って増やさない)
- **ミラの【グッドフェイス】は `RecordOnly`** — 中ディレイの効果はスキル側が持っているので、
  ここで数えると二重になる(下の「キャラスキルの器を 1 本にする」参照)
- **アイコンは 243 枚とも wiki から取り込んだ**(`?plugin=ref&page=…&src=…`)。
  ページごとの添付なので、キャラごとにそのページから引く
- **消えた id は中ディレイ側でも落とす** / 【一閃】を `actual_delay` から外した結果、
  保存済みの `boris_mastery_issen` が「未知の中ディレイ減少スキルです」で計算を止めた。
  `migrate_removed_buffs` をバフと中ディレイ減少スキルの両方に広げた


## 2026-08-27 キャラスキルの器を 1 本にする(極・呪われた魔剣 / 極・スパート)

キャラのパッシブ・自己バフ・味方バフが **効き先ごとに 3 つの器**に散っていた:
ステ上昇はバフカタログ(`BuffGroup::CharacterSkill` / `AllySkill` の 9 件)、中ディレイ減少は
`ActualDelaySkillDef`(7 件)、攻撃ダメージ増加は器そのものが無い。この形だと
**1 つのスキルが複数の効き先を持てない**(極・スパート = AGI +10% + 中ディレイ減少、
極・呪われた魔剣 = 攻撃ダメージ +5% + 被ダメージ +5%)。

`crates/domain/src/character_skill.rs` に `CharacterSkillDef` を立てて 1 本にまとめ、
効き先は `SkillEffect`(旧 `MasteryEffect`)をマスタリーと共有する。カタログは
`crates/gamedata/src/character_skills.rs`(17 件)。

1. **効果はスキルが持ち、マスタリーは差し替えの選択子にする**(`mastery_overrides`) /
   wiki のカテゴリ表がその形で書かれている:
   ...
   中ディレイ倍率B
   |ミラ|スパート|-25%/-15%/-5%/-0%||          ← 素は減衰して定常値にできない
   |~   |~      |-5%|マスタリー【グッドフェイス】|
   ```

   逆(マスタリー側に効果を持たせる)にすると、実際に起きたように「スキルは別カタログ、
   そのマスタリーは `RecordOnly`」とコメントで繋ぐしかなくなる。同じ二重計上ハックが
   **ミラ【グッドフェイス】・ロアミニ【ア・プチ】【パウアトゥン】・ジョシュア【エリート】の
   4 件**あり、すべてこの形に直した
2. **マスタリーは攻撃スキルの性能も書き換える**(ノクターン【クイックチャージ】=
   `<レーザーカノン>` のチャージ速度と攻撃ダメージ)。同じ形 — 効果は `Skill` 側、
   マスタリーは選択子 — で足せるが、**該当データがすべて `RecordOnly` の今はフィールドを足さない**
3. **効果は複数持てる**(`effects: &[SkillEffect]`)。極・呪われた魔剣の被ダメージ増加のように
   片方だけ未配線のときは `RecordOnly` を併記する
4. **`effects` が空 = マスタリーを取ってはじめて効果が出る**(ロアミニの M3「スキルを選択」、
   ジョシュアの憑依モード)。`requires_mastery` のような別の仕組みは足さない
5. **`StatRate` は層を持つ**(旧 `MasteryEffect::StatRate` は倍率B 決め打ちだった)。
   極・ソウルゲートは割合増加、極・エンカレッジは倍率B で、供給源ごとに違う
6. **味方スキルはマスタリーの差し替えを見ない** / 同行者のマスタリーは分からないので基本効果のまま
7. **`preview_effective_stats` にマスタリーとキャラスキルを通した** / キャラ画面のプレビューは
   `build_modifiers` しか通っておらず、**マスタリーのステ倍率が乗っていなかった**。
   キャラスキルをバフカタログから外すと同じ穴に落ちるので、両方をここで適用する
8. **極・スパートの AGI +10% は `RecordOnly` のまま** / wiki「能力値増加/減少カテゴリー」の表に
   無く**層が分からない**。層を決め打ちすると数字が黙ってずれる
9. **移行は 1 本の migration** / `stat_sources.actual_delay_skills` を `character_skills` に
   移し、バフとして持っていた 8 件は新 id に読み替える(`joshua_elite_swordsman` →
   `joshua_possession_swordsman` 等)。`MOVED_BUFF_TO_CHARACTER_SKILL` に表がある

### カテゴリX を X3 / X4 / X5 に分けた(2026-08-27)

wiki の X は **X1〜X6 の合計で、上限が副カテゴリごとに違う**。1 つの `AttackDamageRate` に
合流させていたので、片方が上限に張り付いていてももう片方が伸びてしまっていた。

| 供給源 | カテゴリ | 上限 |
|---|---|---|
| 称号「ダメージ増加」 | X3 攻撃ダメージ(基本発動) | +80% |
| キャラスキル・マスタリー | X4 攻撃ダメージ(スキル) | +65% |
| ランダムオプション(サブアーム / レリック) | X5 攻撃ダメージ(特殊) | wiki は「+??%」= 未記載 |

- **親 X は「子に上限を掛けてから足した値」として読み出す**(`CategoryTotals::value`)。
  親自身に上限は無い。親への `add` は `debug_assert` で弾く — 供給源は必ず子に足す
- **X5 の上限は決め打ちしない**。wiki が `+??%` と書いていて値が分からない
- **X1(イザベル)・X2(一般)・X6(日本独自)は未収録**。消費アイテム由来で、いまバフカタログに
  入っている常用バフは「攻撃力増加(ステ)」であって「攻撃ダメージ増加」ではない
- `DamageInput::mastery_attack_damage_rate` は `skill_attack_damage_rate` に改名した
  (キャラスキル + マスタリー = X4 の枠)
- `CategoryTotals::values` は 30 → 33 要素になり、serde が配列を扱えるのは 32 までなので
  `[f64; N]` から `Vec<f64>` にした

### 極・スパートの AGI +10% は倍率B(2026-08-27)

wiki の「能力値増加/減少カテゴリー」の表にスパートの行が無く層が分からなかったが、
**実測で確定した**(ユーザー提供、素ステ AGI 271):

```text
スパート前 375 → 後 412(+37)
倍率B      floor(375 × 0.10) = 37   ← 一致
割合増加    floor(271 × 0.10) = 27   ← 合わない
```

移動速度 +10 は与ダメージ・能力値に入らないので `RecordOnly` のまま。

### [X4] 攻撃ダメージ(スキル)を全キャラ取り込んだ(2026-08-27)

wiki ステータス `[X4]攻撃ダメージ(スキル)` の 71 行を読み、キャラスキル **49 件**を追加した
(カタログは 16 → 65 件)。取り込みの規則:

1. **スキル名が「マスタリー【○○】」の行は `masteries.rs` 側**(マスタリー単体で効くもの)。
   それ以外は**スキル + `mastery_overrides`**(実体はスキル、マスタリーは条件)
2. **数値はカテゴリ表(= 各キャラの「スキル性能一覧」)を正とする。**
   キャラページの「**マスタリースキル**」の節には **Ver8.20 前の古い値が残っている**:

   | | スキル性能一覧 | マスタリースキルの節 | [X4] 表 | 採用 |
   |---|---|---|---|---|
   | シベリン【バーサーク-攻撃型】 | +5% | +30% | +5% | **5%** |
   | シベリン【バーサーク-バランス型】 | +3% | +20% | +3% | **3%** |
   | シベリン【暴走】 | +20% | +60% | +20% | **20%** |
   | シベリン【不屈】 | スタック÷2(最大+5%) | スタック数(最大+10%) | +1〜5% | **5%** |
   | シベリン【槍術】 | +5% | +10% | +5% | **5%** |
   | ルシアン【強剣】 | +5% | +10% | +5% | **5%** |
   | ミラ【ヴァイパーズアイ】 | +3%(5分) | 後方攻撃時 +20% | +3% | **3%** |
   | イスピン【ハードトレーニング】 | +2% | +10% | +2% | **2%** |
   | イソレット【ヨズレル】 | +5% | +10% | +5% | **5%** |
   | ジョシュア【アノーイングネイバー】 | +3% | +10% | +3% | **3%** |
   | ロアミニ【ア・プチ】 | +3% | +10% | +3% | **3%** |

   **スキル性能一覧とカテゴリ表は 11 件すべてで一致し、食い違うのはマスタリーの節だけ。**
   ジョシュアのページには「Ver8.20：マスタリースキル『アノーイングネイバー』の<幽霊>の
   スタック数比例を削除」という更新履歴が残っていて、節の値が更新から取り残されたと分かる。
   一度マスタリーの節を正として取り込みかけたが、値が [X4] の上限 +65% に対して不自然に
   大きくなった(暴走 +60%)ため wiki を読み直して差し戻した。

   **ただし節が「必ず古い」わけではない** — ボリス【シルバースカル優勝者】は同じ Ver8.20 の
   「スタック数比例を削除」が節にも反映されていて性能一覧と一致する。追随度は行ごとに違うので、
   **突き合わせ先は同じページの「スキル性能一覧」**とする(カテゴリ表はそこから作られている)
3. **範囲で書かれている値は最大値**(ユーザー確定 2026-08-27)。アナイス ルシベアバリア
   +1〜3% → 3%、ハードウエポン +5,13〜20% → 20%、シベリン【不屈】+1〜5% → 5%
4. **「デバフ・デバフのデメリット効果」節は収録しない** — イスピンの `<プシーキーの権能>`、
   シベリンの挑発は**対象の**攻撃ダメージを上げるもので、自分の火力には入らない
5. **アナイス【くまさんアングリー】は `RecordOnly`** — wiki が「効果なし(2024/2/21〜)」と書いている
6. **ドタバタペアは効き先が 2 つ**(中ディレイ −5% + 攻撃ダメージ +2%)。器を 1 本にしたので
   1 エントリで両方持てる — 分かれたままなら表現できなかった
7. **`masteries.rs` のモデル済みは 11 → 12 件**。実体をスキル側へ移した 2 件
   (ボリス【必滅者】・ジョシュア【アノーイングネイバー】)が `RecordOnly` になり、
   3 件(イソレット【ヨズレル】・シベリン【不屈】【暴走】)が入った
8. **ステ増加系は表と一致していた** — ステータス「能力値増加/減少カテゴリー」と突き合わせ、
   ベンヤ 極・ソウルゲート(割合増加 AGI +5%)、ロアミニ 極・ア・プチ / 極・パウアトゥン、
   シベリン 魅力発散、ティチエル 魔法の先生、ボリス【シルバースカル優勝者】(最大 +10%)は
   すべて現状のままでよい。「剣を扱う技術」(ボリス/マキシミン M4)とジョシュア【エリート】は
   能力値カテゴリ表に無い(あの表はキャラスキル = バフを並べたもので、パッシブのマスタリーを
   載せていない)が、**各キャラのスキル性能一覧と節が一致している**ので現状の値で正しい

### 効き先を `DamageCategory` で一般化し、バフにも共有させた(2026-08-27)

`SkillEffect` の効き先を `AttackDamage { percent }`(カテゴリX4 決め打ち)から
**`Damage { category: DamageCategory, percent }`** に一般化した。上限はカテゴリ側
(`DamageCategory::cap`)が持つので、効き先を足しても上限のロジックを書き足さなくてよい。

同じ `SkillEffect` を**バフカタログにも持たせた**(`BuffDefinition::damage_effects`)。
これで **1 つのバフがステと与ダメージの両方に効く**形が表せる:

| バフ | 能力値 | 与ダメージ |
|---|---|---|
| 守護者のためのポーション | 割合増加 +10% | L 最終ダメージ +10% |
| クラブSエフェクト | 固定値 +20 | L 最終ダメージ +5% |
| ユキダルマン族の特製ポーション | 割合増加 +30% | X1 +20% |
| テイルズウィーバーのエネルギー | 倍率A ×1.10 | X2 +5% |

**片方しか入れていなかったのは器の問題**だった — `BuffTarget` の 4 変種がすべてステで、
与ダメージの効き先を持てなかった。キャラスキルで解いたのと同じ構造。

1. **カテゴリX を X1〜X6 の 6 副カテゴリに分けた**(上限 +50 / +30 / +80 / +65 / 未記載 / +30%)。
   `DamageCategory::ALL` は 30 → 36 件
2. **ダメージにだけ効くバフは `BuffValue::RecordOnly` + `damage_effects`** で持つ
   (排他枠は押さえるがステには加算しない)。バフカタログは 16 → 34 件
3. **`DamageInput::skill_attack_damage_rate`(f64)を `damage_contributions`
   (`Vec<(DamageCategory, f64)>`)に置き換えた**。キャラスキル・マスタリー・バフを
   `damage_contributions_of` でまとめて渡す
4. 新規収録: **[L] 最終ダメージ** ソウルリンク +20% / 古代レリック +15% / ベンヤ ダークブレッシング
   +100%(上限 45% で頭打ち)、**[X1]** 6 件、**[X2]** 7 件、**[X3]** フィーバー・深化ルーン、
   **[X6]** 略奪パン・茹でミミック、**[E1] スキル倍率増加(割合)** イサック【プリチャージ】+50% /
   ロアミニ【良心】+50% / イェフネン 3 件。キャラスキルは 65 → 71 件
5. **範囲値は最大値**(ユーザー確定)。ソウルリンク +4〜20% → 20%、狂戦士のルーン → 10%、
   古代ガナポリーマナの破片 +1〜15% → 15%

### アビリティの追加効果「ダメージ増加」は X3(2026-08-27)

wiki「装備システム/アビリティ」の表には**「効果」列と「追加効果」列**があり、
**R- 以上の段には追加効果「ダメージ増加 +3/+4/+6%」が付く**。うちのカタログ 28 件
(4 系統 × 7 段)は「効果」列(装備攻撃力)しか取り込んでおらず、**既に持っている装備の
追加効果を丸ごと落としていた**。`EquipmentAbilityDef` に `damage_effects` を 1 フィールド
足すだけで入った(器は `SkillEffect` を共有)。

```text
武器 R/L/E-尖った刃  突き攻撃力+7/8/9  + ダメージ増加 +3%/+4%/+6%  → X3
```

- **`W` 攻撃ダメージ(基本発動)(固定値)は器も式も既にあった** — `DamageCategory::
  BasicTriggerDamageFixed` は上限 1,000 付きで定義済み、式にも `… + g(W)` で入っている。
  未配線だったのは**供給源のデータだけ**。ステータス表の「アビリティ|ダメージ増加|+5〜7,000」は
  **装備システム/アビリティのページに載っていない**(アビリティ素材方式・アイテム方式の別ページか、
  装備強化システム由来)。出典が取れるまで未収録
- **アビリティは武器部位にしか付けられない実装**(`Equipment::base_totals` は
  `parts.weapon.abilities` しか見ない)。手アビリティ(致命打/的中剣の X3 +3〜4%)と
  兜アビリティ(スキル攻撃力増加 → E2)は部位対応から必要


### 装備アイテムそのものの装着時効果を `EquipmentItem::damage_effects` で持つ(2026-08-27)

アビリティの追加効果と**同じ `SkillEffect::Damage` の並び**を `EquipmentItem` に足した。
効き先はステータス表のカテゴリ表が正で、数値と装備補正は各 Item ページから取る。

| カテゴリ | 入れたもの |
|---|---|
| `X5` 攻撃ダメージ(特殊) | エフェクト「装着時攻撃力が3%増加」= 【年占】†幻獣 5 種 + 【18th】†記念の祝福紋様 5 種 |
| `X6` 攻撃ダメージ(日本独自) | コラボ武器 †ニバンボシ(刀/太刀)+3%、手 †ゴリラのあーむかばー / †タヌキの手袋 +5%、†イヅツミの手甲 / †リンの手袋 +3%、エフェクト「一定確率で 3% 上昇」9 件 + †ロスト「装着時 +1%」 |
| `Old` 攻撃ダメージII | 体 †主天使の羽 / †シグマウィング +25%(要塞占領報酬) |
| `O` 物理/魔法ダメージ増加 | 鎧 †リナの服 +3% |

装備カタログは 20 → 49 件。決めたこと:

- **集計は `gamedata::item_damage_contributions(&Equipment)` に置く**。`Equipment::
  ability_damage_contributions` と同じ役割だが、`EquipmentItem` は `Source` / `WeaponClass` を
  持つので domain には移せない。gamedata → domain の依存方向は保たれる
- **†リナの服の「魔法での与ダメージ+3%」は依存で分けない**。wiki が「熊は物理攻撃だが
  適用される」と明記しているので、カテゴリO にそのまま入れる
- **「一定確率で」のものも発動前提で入れる**(ユーザー確定 2026-08-27:「ほぼ発動するので
  発動する前提で OK」)。エフェクトの X6 +3%(Lv310 帯 9 件)を収録した。wiki の文言は
  「攻撃ダメージが3%上昇」「攻撃力が3%上昇」で揺れるが、ステータス表 1205 行はどちらも
  同じ X6 +3% の行にまとめているので同じ効き先にする。
  **確率でも入れられないものは残る** — †ドゥクスのアームカバー / †千尋の腕時計 /
  †ミグミグ族の服 は wiki が「[ ]ダメージ増加(**分類不明**)」の節に置いていて
  効き先のカテゴリが決まっていない。確率とは別の理由なので未収録のまま
- **AF「全攻撃ダメージ増加」は未収録**。wiki が「+x%」としか書いておらず値が取れない
- 画面では部位行と装備選択の候補行に「攻撃ダメージ +3%」のバッジを出す。装備補正値の
  列には出ない値なので、出さないと**選んだ瞬間に数字が動く理由が画面に無い**

### `W` の供給源は「新装着アビリティ」だった(2026-08-27)

`Item/合成/装着アビリティシステム/新装着アビリティ` に載っていた。**既存の武器アビリティ
カタログ(装備システム/アビリティ の (下)〜E- 段)とは別のシステム**で、こちらは
古代精霊 / 深淵 / 喪失 / 夜星 の 4 段。追加アビリティとして
`ダメージ増加 +5,000〜10,000`(= `W`)と `ダメージ増加率 +8〜11%`(= `X3`)を持つ。

追加アビリティは wiki が並べている候補プール(6 種前後)から**ランダムに 2 つだけ**付くため、
装備登録に「枠を選んで中身と実測値を選ぶ」器を追加した。固定ダメージは `W`、割合は `X3`、
突き・斬り・魔攻・魔防・命中は装備補正へ加算し、HP/MP 回復は計算の消費先がないため記録だけ行う。
同じページに**手アビリティ**(致命打 / 的中剣)と**鎧アビリティ**もあるので、
「アビリティを部位ごとに持つ」も同時に決める必要がある。別 goal にする。

### 残っている穴

- **アビリティは武器部位だけ** — 手アビリティ(致命打/的中剣の X3 +3〜4%)と
  兜アビリティ(スキル攻撃力増加 → E2)は、部位ごとのアビリティ対応から必要
- **手・兜・鎧の新装着アビリティは未収録** — 武器のカテゴリー4と追加2枠は収録済み。
  手の `X3`、兜の `E2`、鎧の防御効果は部位ごとの候補整備が必要
- **AF「全攻撃ダメージ増加」(`X5`)は wiki が「+x%」で値を書いていないので未収録**
- **`T` ダメージ増幅** — 3 コンボ以上 −10% は実装できるが、武器ランダムOP は確率発動で `RecordOnly` 方針
- **味方スキルは相手のマスタリーを見られない** — イスピンの極・エンカレッジは
  マスタリー【アンパッサン】で攻撃ダメージが付くが、同行者の選択が分からないので入れていない
- **`masteries.rs` の 243 件はマスタリーの節から取り込んだままで、数値を性能一覧と
  突き合わせたのはモデル済みの 12 件だけ**。`RecordOnly` の 231 件を計算に入れるときは、
  note の数値をそのまま使わず**必ず性能一覧と突き合わせる**

## 2026-08-27 称号の条件付き追加ダメージ

出典: wiki「称号/special」(取得 2026-08-27)。

1. **称号の「関連マップ／特定の敵に追加ダメージ」は §5 の新-割合へ入れる** / 1 段ダメージではなく、段数と武器強化追加ダメージを含む合計へ掛ける。無条件の「ダメージ n% 増加」(X3)とは別の効果
2. **テシスコアの `CoreRegion` を条件判定に流用しない** / アークロン地下要塞はコア上は Abyss だが、死の騎士をアビス全域へ効かせてはいけない。`Content::game_region` を独立して持つ
3. **喪失の島は `eclipse_boss` / `eclipse_2` / `eclipse_subjugation` / `lost_forest` に限定する** / アフェティリア等まで「関連マップ」と推測で広げない。神鳥の塒は `shinchou_normal` / `shinchou_hard` / `relic_sanctuary_shinchou`、アークロン地下要塞は同名コンテンツだけ
4. **wiki 表の `~` は備考セルの継承として扱う** / マーキュリアル 5 ボス×6依存とアークロン要塞守護者×6依存は、各グループの全称号が同じ敵限定 +10% を持つ
5. **未収録の対象も称号側の条件は先に構造化する** / プラバ、マーキュリアル各ボス、深淵の使徒は現在の計算対象カタログに無いが、対応する地域／敵 ID が追加された時点で同じ resolver により発動する

## 2026-08-27 クラウンの入力規則

出典: wiki「クラウン」`#gad4c4e9` / `#UsedScore`、公式「クラウンシステムについて」(取得 2026-08-27)。

1. **クラウンは画面に表示される実値を +10 刻みで保存する** / 名声強化は能力値10ポイントごと。ランク名は計算に不要なので入力させない
2. **通常の能力値上限は +100、選択報酬に指定した1能力値だけ +300まで許可する** / ランク選択報酬の最大 +200 と名声強化を合わせた上限。選択先を変えたとき、旧選択値が100を超えていれば100へ戻す
3. **選択報酬には `+20 / 260 / 280 / MAX` のショートカットを置く** / `+20` は現在値への加算、`260 / 280 / MAX` は実値の固定。ランク報酬だけでなく通常強化 +100 を含む最終値をボタンに表示し、その他は各能力値の ±10 で調整する

## 2026-08-27 装備登録と装備強化の入力規則

1. **装備は部位ごとの登録一覧と選択中IDで持つ** / 計算は選択中だけ、保存検証は登録全件。旧JSONはstorage v8で各部位1登録へ一度だけ移行する
2. **装備本体はカタログ上限を自動設定し、表示値を押したときだけ例外編集する** / 全行を切り替える独立した「装備補正を編集」ボタンは置かない。通常時は入力欄に見えないプレーンな読み取りテキストとし、触れた値だけ同じ寸法の入力面へ変える。「ユーザーが埋める欄」ではなく「カタログ値の例外上書き」と認知でき、編集開始・終了でも行の高さを変えない。装備本体とエンチャントは同じ9値の行で横に並べる。エンチャント枠は `wiki上限 - カタログの装備本体最大` で確定する装備固有値であり、実物の装備本体を編集しても変えない。改・セイクリッドブレードは `840 - 530 = 310`、実物が520なら結果は830
3. **属性強化は装備に保存しない** / キャラで選択中の属性を対象の実装備へ部位ごと +9 固定で反映する。無属性・未選択・未装備は0
4. **+12〜+15は等級を保存する** / 確率区分10/30/70/95/100%の上端へ対応させ、倍率端数は四捨五入する。鎧も武器と合算する
5. **新装着アビリティはランダム追加2枠を実測値で保存する** / 選択したカテゴリー4の段に応じて候補とレンジを絞る。固定ダメージは `W`、割合は `X3`、突き・斬り・魔攻・魔防・命中は装備補正へ反映する。HP/MP回復は記録のみ
6. **カタログ外装備は補正式の種別だけ選ぶ** / 武器6系統、鎧5種を登録データに持ち、基本値+エンチャント値から固定ダメージを自動算出する。添付の魔鎧は「魔鎧・+15・最上」で1,984,400
7. **旧実測固定ダメージは最も近い等級へ確定移行する** / 旧値そのものを互換フィールドとして残さず、同じ基本値・エンチャント値から出る5等級のうち差が最小の等級へ変換する。式を特定できない旧カスタム品だけは最上へ寄せる
8. **消えた旧アビリティIDは起動時に除去する** / 現行カタログにあるIDは保持し、消えたIDは装備以外の編集・保存を妨げないよう冪等移行で落とす
9. **装備の選択はアイコンを主識別にする** / Tale Wiki各装備ページの添付画像を同梱し、部位一覧・登録切替・候補一覧に同じアイコンを出す。画像を特定できない装備だけ中立の `?` を使い、別装備の画像を推測で流用しない
10. **登録オーバーレイは明示操作でだけ閉じる** / 背景の誤クリックや装備候補の選択では閉じず、×または Escape で閉じる。装備候補を選んだときは候補一覧だけを畳み、編集中件数を上部に常設する
11. **武器アビリティは3スロット・同カテゴリー1件で持つ** / wiki の武器スロット数は3。同じ「鋭い刃」系でもカテゴリー1の下級斬りとカテゴリー4の夜星の鋭い刃は併用でき、カテゴリー3の暴風の刃を加えた3件が実例。旧決定の「効果系統ごとに1件」は誤りとして更新し、保存検証・起動時正規化もカテゴリー単位へ変更する
12. **装備を選んでも登録編集画面は閉じない** / 候補一覧だけを畳み、直下のエンチャントへ続ける。登録名は装備選択から独立させ、エンチャントを主カード、基本値を参照値と例外編集にする
13. **武器ディレイ系アビリティはDPS計算へ入れない** / 疾風の刃・暴風の刃などの武器ディレイ増減は前/後ディレイに作用し、中ディレイ減少ではない。現環境では後ディレイがほぼ 0 になるため、選択と効果表示だけを行い計算対象外とする。前ディレイも同様
14. **ボリスの代表3スキルは武器種を直接絞る** / 極・連は刀、極・残影斬は太刀、極・氷撃斬は大剣。依存ステータスだけでは大剣を特定できないため、その他のスキルだけ従来の武器系統フィルタへフォールバックする。「すべて見る」で手動選択も許可する
15. **基本値の隣にエンチャント込み合計を常設する** / 編集の主対象はエンチャントなので、基本・エンチャント・合計を同じ行で読み比べられるようにする。合計が変わったときだけ bump する
16. **装備補正は S/H/I/M を先頭4枠にし、残り5補正は既定で畳む** / 通常エンチャントするのは突き・斬り・魔攻・魔防。表示はゲーム内に合わせて `合計(+エンチャント)` とし、同じ行にアビリティ加算も併記する
17. **武器アビリティ本体もランダム追加もプルダウンを使わず、候補チップで選ぶ** / 本体はカテゴリーごとの排他選択なので `基本補正(カテゴリ1) / 新装着(4) / 武器ディレイ(3)` の行を常設し、武器系統に合う収録全件を効果順に並べ、選択中の1件だけを強調する。ランダム追加2枠は順序のない複数選択なので、候補チップを押すと `種類 / 実測値 / 外す` の1行が積まれる。数値列は種類名を重ねず同じ開始位置に揃える。自然回復系は表示しない。追加行の面は濃いインセット色で塗らず、白に近い薄い面と枠で親子関係だけを示す
18. **通常装備の追加5補正は武器・鎧の共通経路で開閉を動かす** / `物防・命中など5補正` は部位別実装にせず同じ行生成を使う。追加5行へ220msの入退場を付け、展開ボタン自体は動かさない。成長装備は全補正を常設するためこの開閉を持たない

## 2026-08-27 キャラ固有の腕装備補正変換

出典: Tale Wiki `Skill/ボリス`、`Skill/マキシミン`、`Skill/ナヤトレイ`、`Skill/ミラ`、`Skill/イサック`、`Skill/ベンヤ`、`Skill/ロアミニ`(取得 2026-08-27)。全19キャラのパッシブ節を横断確認した。

1. **エンチャント枠は強化能力値のまま保持し、パッシブによる派生値だけを基本能力値へ加える** / ボリス・マキシミンは腕装備の突き `(基本+エンチャント+盾のシエナのオーラ)×1.0` を魔攻基本へ変換する。シエナの武器／盾能力値も仕様上エンチャント扱い。エンチャントそのものを基本値へ移動するとモデル上の枠が壊れるため、`character_wrist_base_bonus` が計算入力用の派生値を返す
2. **腕装備は Tale Wiki の区分を `WristType` としてカタログに持つ** / バンド系パッシブを名前の部分一致やキャラIDだけで判定しない。盾＋は別スロットで `WristType` を持たず、対象外
3. **バンド変換は敏捷 `(基本+エンチャント)×0.7` の整数切り捨て** / ナヤトレイ・イサックは突き／物理複合型→突き、斬り型→斬り。ミラ→斬り、ロアミニ→魔攻。ベンヤは素HACKと素MRの大小で斬り／魔防へ分け、同値は wiki に規定がないため加算しない
4. **型は主軸スキル依存を登録済みの型情報として使う** / 主軸未選択時のみ、現在計算中のスキル依存へフォールバックする。ダメージ計算とコンテンツ評価の両方で同じ resolver を通す

## 2026-08-27 シエナのオーラは装備と独立して登録・装着する

出典: wiki「装備システム/シエナのオーラ」`#w4fdd1bb`「抽出・注入」(取得 2026-08-27)。抽出すると装備の取引属性が元に戻り、抽出済みオーラは取引・OM利用ができ、オーラの付いていない同じ部位の装備へ注入できる。

1. **旧決定「`EquipmentPart::siena` に持つ」は更新する** / オーラは特定の装備品に固定された構成要素ではなく、抽出後も同部位用の独立した所持物として残る。`Equipment::siena` に8部位それぞれの登録一覧と装着中IDを持たせ、装備登録の選択とは切り離す
2. **計算へ入るのは装着中の1件だけ、保存検証は登録全件** / 未装着は `selected_id = None`。登録内容は消さず、装備攻撃力・ステ・New1・防御倍率・中ディレイ・クリティカル率のすべてから外す。存在しないID、0/重複ID、未選択を含む登録内容の部位・値域違反を拒否する
3. **UIは装備登録と同じ操作語彙にする** / 部位一覧に登録数・装着中名称・段階・要約を出し、その場で未装着／登録済みを切り替える。編集は背景クリックでは閉じず、常設の「閉じる ×」か Escape だけで閉じるオーバーレイ。登録名を能力値編集から分離する
4. **旧データの互換移行は作らない** / サービスイン前でデータ変更可というユーザー判断に従い、旧 `EquipmentPart::siena` は削除して新しい空の登録一覧として読む。二重所有やフォールバックを残さない
### 装備カタログは各 Item ページの最後のインファーナルより後を収録する（2026-08-27）

装備選択肢は Tale Wiki `Link/装備Item` から辿れる部位別ページを正とし、各ページで最後に現れる
インファーナル装備より後の行を「インファーナルより強い」の収録境界とする。通常品だけでなく
イベント・コラボ品も含める。9補正または装備本体との総上限が `?` の行は推測せず、Wiki更新待ちとする。

既存の手検証済みデータは装着時効果のカテゴリ判断を含むため同名なら優先する。盾＋だけは通常の
一覧を持たず、ユーザー指定のライジングホリックカフス1件に固定する。これは完成値140の装備ではなく
成長装備として扱い、初期入力は全補正140、各補正の上限は200とする。UIには全補正140の固定入力を置く。

Wiki行に実画像参照がある装備は安定IDと同名で同梱する。参照がない装備は別画像を推測せず、規格どおり
破線の `?` を出す。

### 効果・AF・レリックの候補と成長値（2026-08-27）

出典: Tale Wiki「Item/Effect」「Item/Artifact」「Item/Relic/Right」「Item/Relic/Left」
（取得 2026-08-27）。

1. **効果は21周年4系統と22周年スタリオンサイン4色を収録する** / 22周年品の主能力総上限700は公式告知 no=154958、その他8補正の総上限255はユーザー確定(2026-08-27)。装備本体との差分を固定エンチャント枠にする。例: ブルーの突きは `700 - 30 = 670`、命中は `255 - 35 = 220`
2. **AFは候補の依存能力と、装着時効果の適用条件を別に持つ** / `recommended_dependency` は主軸スキルに合う候補を先に見せるための表示情報。`damage_dependency` は「斬りスキルの攻撃ダメージ」等を一致するスキルだけに反映する計算条件。候補を絞っただけで他依存のスキルへ効果を漏らさない
3. **無条件の「攻撃ダメージ20%増加」はX5、特定依存の20/30%増加はP** / 既存の装備アイテム効果と同じカテゴリへ接続し、コンテンツ評価では計算中の各スキル依存で判定し直す
4. **レリックは神鳥+1〜+10、続いてルナリア+1〜+10の20段階を左右それぞれ持つ** / Tale Wiki「神鳥のレリック・ルナリアレリック」`#new_relic_item`（取得2026-08-28）。各段階の下限は直前段階の全補正MAX、上限は表の当該段階MAXとする。神鳥+2なら右はS/H/I 30〜50・命中/Cri 25〜45、左は物防/魔防30〜50・回避/敏捷25〜45。ルナリア+10は全対象補正190〜200。エンチャント不可。アビリティ1枠・付加オプション2枠はルナリアだけで、神鳥には表示・保存しない
5. **AFは6依存すべてに「○○力 - ディフェンシオ」を1件以上出す** / エクリプスで確定値が揃う突き・斬り・魔防を優先し、物理複合・魔攻・魔斬は数値と20%効果が確定しているアークロンを収録する。物理力ディフェンシオのWiki上限行にある欠落セルは、装備補正が同一のリストア/スピーディー行の上限を採用する
6. **主要3段はプシーキー・エクリプス・エーテリアルチューブを各6依存×通常版/ディフェンシオで揃える** / プシーキーの20%とエーテリアル・ディフェンシオの35%はWiki確定値としてPへ接続。エクリプスは同系列で確定している斬り・魔防と同じ30%を6依存へ適用する。通常エーテリアルの効果値はWikiが`??`のため未計算。エクリプスとエーテリアルの未記載補正は同系列の規則を使い、各Source注記へ補完であることを残す
7. **AFの耐久効果は攻撃効果と分離して保持する** / Tale Wiki「Item/アクセサリー用装備/アーティファクト」`#he1313ff`、`#apple_af`、`#akron_af`、`#Eclipse`、`#DistortedVillage`（取得 2026-08-28）。エーテリアル通常はダメージ緩和+15%、ディフェンシオは+40%。プシーキー/アークロン/エクリプス通常は2024年追加の緩和+10%。ディフェンシオの「防御力+15」「防御力+30%」は緩和とは別の効果種として記録する。これらをプレイヤーの与ダメージ計算へ混ぜず、装備候補のバッジに併記する
8. **AF画像は同名のWiki添付だけを使う** / 45件中、実体を取得できた35件を安定ID名で同梱する。`&null{&ref(...)}`だけが残るエクリプス5件と、個別添付がないエーテリアル5件の計10件は通常版の画像を複製せず、未収録の`?`を表示する
9. **レリックは20段階を20件の候補カードとして並べず、種別→強化段階で選ぶ** / 検索語で見分ける装備ではなく、神鳥・ルナリアと+1〜+10の直交する属性。種類変更時は現在の強化段階を維持し、選択面を閉じずに補正値を即時反映する
10. **成長装備の±操作は強化段階ではなく、各装備補正値に置く** / 神鳥・ルナリアの段階は `+1〜+10` から直接選ぶ。選択中段階の下限〜MAX内で調整する突き・斬り・物防などの各実値に、それぞれ−/+を常設する。ライジングホリックカフスも同じ共通入力を使う
11. **装備候補の切替時は選択面を動かさず、下の詳細領域だけを差し替えて動かす** / 選択中の装備名は短くflashし、補正・アビリティ・ランダムOP・装備強化をまとめた詳細領域は200msで入れ替える。神鳥/ルナリアだけでなく、段階変更と全装備候補に同じ共通経路を使う
12. **装備登録とキャラクターは掴んで表示順を変えられる** / 装備登録順はキャラの装備データ内の配列順として保存し、キャラクターレールの順は計算・ドメインに影響しない端末内の表示設定として保持する。削除は複数登録で別の装備へ即時切替すると結果が見えにくいため、編集画面内に登録一覧を常設し、明示的な2回押しにする
13. **通常装備の各エンチャント値は+12/+14/+17/+20を1押しで加算できる** / 強化カード1個分の代表値を繰り返し手入力させない。各ボタンは現在値へ加算し、装備ごとのエンチャント上限を超える場合は上限で止める。MAXの左に固定位置で常設し、成長装備の装備補正には出さない
14. **通常エンチャントを持つ全部位の主要補正に、右余白で上限までの推奨内訳を出す** / 対象部位は武器・鎧・兜・盾・頭・体・手・足・効果・AF。武器は武器種・武器系統を優先し、刀=突き・斬り(火力係数にSも入る)、太刀=突き・斬り、大剣=斬り・魔攻など。AF等に専用依存があればそれを使い、汎用装備は選択中の主軸スキル依存を使う。鎧・盾のように該当攻撃枠がない品は、エンチャント可能な物防・魔防・回避へ案内する。主軸スキル未選択時も、効果は最大枠の系統、その他はエンチャント可能なSHIMへ案内して空表示にしない。基本は+17を反復し、1回減らせる場合だけ必要最小数の+20へ置き換える。費用面から端数+18/+19は提案しない。例: 残り45は`+17×2 + 端数11 / 3回`、残り56は`+20×2 + 端数16 / 3回`。現在値が変わるたび再計算し、上限では強化完了を出す
