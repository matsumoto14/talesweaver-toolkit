...
**そのまま足せない**: 追加アビリティは wiki が並べている候補プール(6 種前後)から
**ランダムに 2 つだけ**付く。器はランダムオプションと同じ「枠を選んで中身を選ぶ」形が要る。
...
- **`W` は出典が取れた(新装着アビリティ)が器が無い** — 追加アビリティは候補プールから
  ランダムに 2 つ付くので、ランダムオプションと同じ「枠」の器が要る
