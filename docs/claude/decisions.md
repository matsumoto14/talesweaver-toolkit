...
5. **キャラのパッシブは新しい補正源にした** / `StatSources::actual_delay_skills`(9 件のカタログは gamedata)。ステ補正ではないのでバフカタログには入れられず、`ElementSources` と同じ「`StatSources` にぶら下げるキャラ紐付けの供給源」に揃えた。`StatSources` は JSON 列なので **DB マイグレーション不要**
