...
CONTROL_SEL = re.compile(r"(?:^|[\s,>])(?:input|button|select|textarea)|\.(?:btn|chip|tab|field|check|toggle|max-btn|num-field|pill|badge)(?![\w-])")

HEX = re.compile(r"#[0-9A-Fa-f]{3,8}")
# 面としてべた塗りしている背景。グラデーションや影の中の色は「面」ではない
FLAT_BG = re.compile(r"background(?:-color)?\s*:\s*#[0-9A-Fa-f]{3,8}", re.I)
