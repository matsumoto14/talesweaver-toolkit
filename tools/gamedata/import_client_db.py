...
- `c35_EquipSlot_*`          : 装備部位。1=兜, 2=武器, 3=鎧, 4=盾(腕), 5=頭, 6=手, 7=体, 8=脚,
  13=エフェクト, 15=乗り物, 18=アーティファクト, 20/21=レリック右/左(表 0309 `c4_Slot` と同じ値)。
  本スクリプトが対象にするのは `EQUIP_SLOT_TO_PART_SLOT` に載る 1/2/3/4/5/6/7/8/13/18 だけ
  (15 は実データ 0 件、20/21 はレリックで段成長という別モデルのため対象外 = 既存の
...
DESC_EFFECT_RULES = [
    ...
    (re.compile(r"スキル使用時、?一定確率で攻撃(?:力|ダメージ)が(\d+)%(?:上昇|増加)"), {"Effect"},
     [("damage", "AttackDamageJapan")]),
    ...
    # 言い回しが 2 つある(「与ダメージ+20%、防御力+20%」「ダメージ20%上昇・防御力20%上昇」)
    (re.compile(r"一定確率で与?ダメージ\+?(\d+)%(?:上昇)?[、・]防御力\+?(\d+)%"), {"Artifact"},
     [("damage", "AttackDamageSpecial"), ("survival", "DefenseRate", "percent")]),
]
# 効果文らしいのに規則に掛からなかったものを拾う目印(見落としを stderr に出すためだけに使う)。
# 「被ダメージ」は与ダメージ側ではないので除く。語を狭く取ると言い回し違いを黙って落とす
# (「攻撃ダメージが3%上昇」の旧エフェクト 6 件を落としていた。レビュー指摘 2026-09-21)。
DESC_EFFECT_HINT_RE = re.compile(r"(?:(?<!被)ダメージ|攻撃力)[^。\n]*?\d+%|\d+%[^。\n]*?(?:(?<!被)ダメージ|攻撃力)")
