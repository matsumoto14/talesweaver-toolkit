...
/// 到達バッジで共通(「余裕 / 通る / ぎりぎり / 届かない」)。
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum ReachTier {
    /// 討伐 `COMFORTABLE_SECONDS` 秒以内
    Comfortable,
    /// 討伐 `REACHED_SECONDS` 秒以内
    Reached,
    /// 討伐 `CLOSE_SECONDS` 秒以内
    Close,
    Short,
}

/// 余裕(Comfortable)の境目 = 1 分。ユーザー決定 2026-09-16。
pub const COMFORTABLE_SECONDS: f64 = 60.0;
/// 通る(Reached)の境目 = 3 分。ユーザー決定 2026-09-16。
pub const REACHED_SECONDS: f64 = 180.0;
/// ぎりぎり(Close)の境目 = 5 分。これを超えると Short(届かない)。
/// 10 分で切ると「長すぎて判定にならない」(ユーザー決定 2026-09-16、10 分から変更)。
pub const CLOSE_SECONDS: f64 = 300.0;

impl ReachTier {
    /// 討伐時間(秒)から到達段を決める。討伐時間が出せない(敵 HP 未収録・中ディレイ
    /// 未収録)ときは判定不能として `None`(0 や「—」で埋めて嘘をつかない)。
    pub fn of_defeat_seconds(seconds: Option<f64>) -> Option<Self> {
        let seconds = seconds?;
        Some(if seconds <= COMFORTABLE_SECONDS {
            ReachTier::Comfortable
        } else if seconds <= REACHED_SECONDS {
            ReachTier::Reached
        } else if seconds <= CLOSE_SECONDS {
            ReachTier::Close
        } else {
            ReachTier::Short
        })
...
pub struct ContentEvaluation {
    ...
    pub entry_ok: bool,
    /// 火力の到達段。敵データなし・討伐時間が出せない(ダメージ不明含む)は None
    pub reach: Option<ReachTier>,
    /// 火力が目安(討伐時間)に届いているか。敵データなし(火力不問のコンテンツ)は true、
    /// 討伐時間が出せない(ダメージ不明含む)は false
    pub reaches: bool,
    /// 火力(討伐時間の目安到達)と入場条件の両方を満たすか
    pub clear: bool,
}
...
pub fn evaluate_content(
    ...
    let reach = damage
        .as_ref()
        .and_then(|d| ReachTier::of_defeat_seconds(d.defeat_seconds));
    let reaches = match content.enemy_id {
        None => true,
...
mod tests {
    ...
    #[test]
    fn 到達段は討伐時間の境目で4段に分かれる() {
        assert_eq!(ReachTier::of_defeat_seconds(Some(60.0)), Some(ReachTier::Comfortable));
        assert_eq!(ReachTier::of_defeat_seconds(Some(60.1)), Some(ReachTier::Reached));
        assert_eq!(ReachTier::of_defeat_seconds(Some(180.0)), Some(ReachTier::Reached));
        assert_eq!(ReachTier::of_defeat_seconds(Some(180.1)), Some(ReachTier::Close));
        assert_eq!(ReachTier::of_defeat_seconds(Some(300.0)), Some(ReachTier::Close));
        assert_eq!(ReachTier::of_defeat_seconds(Some(300.1)), Some(ReachTier::Short));
        assert_eq!(ReachTier::of_defeat_seconds(None), None);
    ...
    fn content(has_enemy: bool, requirements: Vec<ContentRequirement>) -> Content {
        Content {
            id: "c".into(),
            name: "テスト".into(),
            enemy_id: has_enemy.then(|| "e".into()),
            requirements,
            core_region: None,
            game_region: None,
            series: None,
    ...
    fn クリア判定は火力と入場条件の両方を見る() {
        ...
        // 火力(討伐時間が Reached の境目以内)・条件とも満たす
        ...
        assert!(e.entry_ok && e.reaches && e.clear);

        // 火力だけ未達(討伐時間が 300 秒を超える)
        let e = evaluate_content(&content(true, vec![]), dmg(300.1), &eq, aw, dep, 0);
        assert!(e.entry_ok && !e.reaches && !e.clear);

        // 条件だけ未達
        let e = evaluate_content(
            &content(true, vec![ContentRequirement::EternalLevel(1)]),
            dmg(REACHED_SECONDS),
            &eq,
            aw,
            dep,
    ...
    fn 敵データなしコンテンツは条件のみで判定する() {
        ...
        let e = evaluate_content(
            &content(false, vec![ContentRequirement::AwakeningStage(3)]),
            None,
            &eq,
            aw,
            None,
        ...
        assert!(e.reach.is_none());

        let e = evaluate_content(
            &content(false, vec![ContentRequirement::AwakeningStage(4)]),
            None,
            &eq,
            aw,
            None,
    ...
    fn スキル未収録はダメージ不明としてクリア不可() {
        ...
        assert!(e.entry_ok);
        assert!(!e.reaches && !e.clear);
        assert!(e.damage.is_none());
        assert!(e.reach.is_none());
    }

    #[test]
    fn 討伐時間が出せないときは段がNoneになる() {
        // 敵データあり・ダメージも出ているが defeat_seconds が None(敵 HP か中ディレイが
        // 未収録)なケース。0 や「届かない」扱いにせず None のまま返す。
        let dmg = Some(BestSkillDamage {
            skill_id: "s".into(),
            per_hit_primary: 1000,
            total_primary: 1000,
            defeat_seconds: None,
    ...
    fn テシスコア条件は合計値で判定する() {
        ...
        let e = evaluate_content(
            &content(false, vec![ContentRequirement::ThesisCoreTotal(0)]),
            None,
            &eq,
            aw,
            None,
