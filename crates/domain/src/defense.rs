...
mod tests {
    ...
    fn 防御力はステ3倍と装備防御6倍() {
        let p = defense_profile(&stats(200, 150, 0), &EquipmentValues {
            ...
            magic_defense: 40,
            ..Default::default()
        }, no_caps(), &RandomOptionTotals::default());
    ...
    fn カット率は1マイナスaを80足したaで割った値() {
        let p = defense_profile(&stats(200, 150, 0), &EquipmentValues::default(), no_caps(), &RandomOptionTotals::default());
    ...
    fn 特殊回避は下限20上限63に収まる() {
        ...
        assert!((defense_profile(&stats(0, 0, 0), &zero, no_caps(), &RandomOptionTotals::default()).combo_evasion - 0.20).abs() < 1e-9);
        // 10 + 150/15 + 200/7.5 = 10 + 10 + 26.666.. = 46.666..%
        let p = defense_profile(&stats(0, 150, 200), &zero, no_caps(), &RandomOptionTotals::default());
        assert!((p.combo_evasion - 0.4666666666666667).abs() < 1e-9);
        // 上限 63%
        assert!((defense_profile(&stats(0, 310, 310), &zero, no_caps(), &RandomOptionTotals::default()).combo_evasion - 0.63).abs() < 1e-9);
    }

    #[test]
    fn 回避Pは15足すAGI1_2倍足す攻撃タイプ別増加() {
        // DEF200 / MR150 / AGI100、STAB+HACK は 0、装備なし
        let p = defense_profile(&stats(200, 150, 100), &EquipmentValues::default(), no_caps(), &RandomOptionTotals::default());
    ...
    fn 回避Pにランダムオプションの回避率増加が足される() {
        ...
        let base = defense_profile(&stats(200, 150, 100), &eq, no_caps(), &RandomOptionTotals::default());
        let with_ro = defense_profile(&stats(200, 150, 100), &eq, no_caps(), &totals);
    ...
    fn 物理の回避P増加は突き足す斬りを100で割って切捨ててから足す() {
        ...
        let p = defense_profile(&s, &EquipmentValues::default(), no_caps(), &RandomOptionTotals::default());
