...
const ITEM_SOURCE_STALLION_EFFECT: Source = Source {
    page: "公式お知らせ no=154958 / クライアント DB dm_00001_0555.csv ItemId 1061128〜1061131",
    retrieved_on: "2026-09-21",
    note: "主能力の総上限700は公式。残り8補正はクライアント DB の実値(対になる能力 500 / その他 255)。\
           wiki は ?? のまま、以前の 255 一律はユーザー報告で誤りと判明(Issue #30)",
};
...
/// カタログは静的データなので、プロセス内で 1 回だけ組み立てて使い回す。`Arc` なので
/// `find_equipment_item` のような検索は複製せずに読める。
static EQUIPMENT_CATALOG_CACHE: std::sync::OnceLock<std::sync::Arc<[EquipmentItem]>> =
    std::sync::OnceLock::new();

fn cached_equipment_catalog() -> std::sync::Arc<[EquipmentItem]> {
    std::sync::Arc::clone(
        EQUIPMENT_CATALOG_CACHE.get_or_init(|| build_equipment_catalog().into()),
    )
}
...
fn build_equipment_catalog() -> Vec<EquipmentItem> {
    ...
    for item in items.iter_mut() {
        ...
        // client DB 由来のときはここまで確定しないので、この後段でまとめて上書きする
