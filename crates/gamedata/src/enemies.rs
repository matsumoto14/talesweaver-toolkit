...
//! - HP(ソロ)は実測表の値。旧リポ twtoolkit の `monsters.json`(28 体)と全件一致。
//!   **PT 時の HP 増加**(実測表 備考「アフェティリア(N) は PT 時メンバー数×5%」など)は
//!   持たない(ソロの値だけを正とする)。討伐時間(HP ÷ 期待 DPS)に使う
...
struct EnemyRecord {
    ...
    /// HP(ソロ)。実測表「モンスター能力値リスト」の HP 列(旧リポ twtoolkit
    /// `monsters.json` の 28 体とも一致)。PT 時の HP 増加は持たない
    hp: Option<i64>,
...
const ENEMIES: &[EnemyRecord] = &[
    ...
    EnemyRecord { id: "siokan_boss", name: "シオカンボス", defense: 35220, damage_reduction: -4550, cut_rate_a: 0.49, element_threshold: 125, agi: None, critical_taken_rate: None, hp: Some(2_000_000_000), client_image: Some("monster_2103477") },
    // wiki 裏取り済み: 防 53500 以下(旧リポ 53220 と整合)、カット率 -51% → 0.49、属性 125、被害減少 有
    EnemyRecord { id: "odin", name: "オーディン", defense: 53220, damage_reduction: -4550, cut_rate_a: 0.49, element_threshold: 125, agi: Some(1500), critical_taken_rate: Some(-450.0), hp: Some(8_000_000_000), client_image: Some("monster_2103496") },
    // 旧リポ(ランキング戦個体)。wiki は通常オーディンのみ記載
    EnemyRecord { id: "odin_rank", name: "オーディン(ランク)", defense: 59220, damage_reduction: -4550, cut_rate_a: 0.49, element_threshold: 125, agi: None, critical_taken_rate: None, hp: Some(12_000_000_000), client_image: Some("monster_2103496") },
    // ---- エクリプス ----
    // 旧リポ。wiki「エクリプス ボス(ソロ)」= 約41640・カット率 -51% → 0.49 と整合(個体差あり)
    EnemyRecord { id: "eclipse_1", name: "エクリプス1", defense: 41220, damage_reduction: -4550, cut_rate_a: 0.49, element_threshold: 125, agi: Some(1420), critical_taken_rate: Some(-350.0), hp: Some(2_550_000_000), client_image: Some("monster_2107777") },
    EnemyRecord { id: "eclipse_2", name: "エクリプス2", defense: 42720, damage_reduction: -4550, cut_rate_a: 0.49, element_threshold: 125, agi: Some(1420), critical_taken_rate: Some(-350.0), hp: Some(2_550_000_000), client_image: Some("monster_2107778") },
    // 旧リポ。wiki「討伐戦ボス」のカット率 -68.5% → 0.315 と一致
    EnemyRecord { id: "eclipse_subjugation", name: "エクリプス討伐戦", defense: 62700, damage_reduction: -4550, cut_rate_a: 0.315, element_threshold: 125, agi: Some(2050), critical_taken_rate: Some(-500.0), hp: Some(17_850_000_000), client_image: Some("monster_2107826") },
    // 旧リポ。wiki 狩り場情報一覧に対応行なし(エクリプス地域の派生コンテンツ)
    EnemyRecord { id: "lost_forest", name: "喪失の森", defense: 37350, damage_reduction: -4550, cut_rate_a: 0.49, element_threshold: 120, agi: None, critical_taken_rate: None, hp: Some(255_000_000), client_image: Some("monster_2107605") },
    // ---- エタ(異界の峡谷 / 最後の決戦) ----
    ...
    EnemyRecord { id: "leitia_n", name: "レイティアN", defense: 106950, damage_reduction: -3250, cut_rate_a: 0.325, element_threshold: 125, agi: Some(2350), critical_taken_rate: None, hp: Some(15_000_000_000), client_image: Some("monster_2108577") },
    EnemyRecord { id: "architect_n", name: "設計者N", defense: 106950, damage_reduction: -3250, cut_rate_a: 0.325, element_threshold: 125, agi: None, critical_taken_rate: None, hp: Some(15_000_000_000), client_image: Some("monster_2108578") },
    EnemyRecord { id: "leitia_h", name: "レイティアH", defense: 132150, damage_reduction: -2925, cut_rate_a: 0.2925, element_threshold: 125, agi: Some(2700), critical_taken_rate: Some(-930.0), hp: Some(15_000_000_000), client_image: Some("monster_2108577") },
    EnemyRecord { id: "architect_h", name: "設計者H", defense: 132300, damage_reduction: -2925, cut_rate_a: 0.2925, element_threshold: 125, agi: None, critical_taken_rate: None, hp: Some(15_000_000_000), client_image: Some("monster_2108578") },
];
...
mod tests {
    ...
    /// HP(ソロ)は実測表の全 42 体ぶんある。旧リポ twtoolkit の `monsters.json`(28 体)と
    /// 重複分は全件一致。桁を取り違えると討伐時間が桁違いに出るので、代表値を固定する。
    #[test]
    fn 敵hpは全件入っていて旧リポと一致する() {
        assert!(ENEMIES.iter().all(|e| e.hp.is_some_and(|hp| hp > 0)));
        let hp = |id: &str| ENEMIES.iter().find(|e| e.id == id).unwrap().hp.unwrap();
        // 旧リポ monsters.json と同じ値
        assert_eq!(hp("tutatur"), 380_000_000);
        assert_eq!(hp("brothers_forge"), 24_500_000);
        assert_eq!(hp("chimera"), 300_000_000_000);
        assert_eq!(hp("relic_sanctuary_20"), 8_128_432_520);
        // 実測表にしか無い行
        assert_eq!(hp("leitia_h"), 15_000_000_000);
        // 聖域 11〜16 はおおむね 100 億から 1 割ずつ増えるが、**13 と 14 は同値**
        // (実測表をユーザーに確認 2026-09-16。等比で埋めないこと)
        assert_eq!(hp("relic_sanctuary_11"), 10_000_000_000);
        assert_eq!(hp("relic_sanctuary_13"), hp("relic_sanctuary_14"));
        assert_eq!(hp("relic_sanctuary_13"), 13_310_000_000);
        assert_eq!(hp("relic_sanctuary_16"), 16_105_100_000);
    }
