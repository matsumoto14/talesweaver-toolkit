<script lang="ts">
  ...
  // どの計算の世代を見るか。面が宣言していなければ攻撃(計算タブの既定)
  const deltaScope = getContext<DeltaScope | undefined>(DELTA_SCOPE) ?? "attack";
</script>
...
{#if delta && motion}
  ...
  <!-- `delta num` は必ずこの class に載せる。action 側で classList に足すと、`deltaClass` が
       変わった瞬間に Svelte が class 属性ごと置き換えて消える(follow の点線・増減の色が
...
<style>
  /* 呼ぶ側が渡す `class`(その画面の見た目)より必ず強くする。
     ...
     クラス名を `tone-*` にしてあるのは、差分枠(`delta num` + `use:delta` が付ける `up` / `down`)と
