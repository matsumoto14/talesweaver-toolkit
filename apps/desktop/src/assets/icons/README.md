...
2026-09-23に極・ダメージプラス(`anais_damage_plus`)を後から収録した際、同じページの詳細見出しの添付
`ダメージプラス.png` を同梱した(テスラコイル等と同じ出どころ)。
...
2026-09-03に装備 1,171 件(`equipment_catalog/client.rs` の全行。セイクリッドと最上位装備を含む。最上位 38 件の数値は 2026-09-14 に client.rs から外して R2 取得制にしたが、アイコンは同梱のまま据え置く。ADR-009)のアイコンをクライアント展開データ
