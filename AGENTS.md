...
- **押した場所は動かない**。クリックした要素の上に何も差し込まない。ドリルダウンは置き換えず右にペインを増やす。重なるもの(候補・ポップオーバー)はレイアウトを押さない。数値欄は `min-width` + `tabular-nums` で桁が増えても幅が変わらない
    ...
    - **動きは `ui/motion.svelte.ts` の部品(§10「動きの部品」)を通す。クラスを画面側で手で当てない。** 値が変わったことを伝える動きは `use:bump` / `use:delta` / `use:swap` / `use:pulse` / `use:flash`(手書きの `classList.add/remove` は機械監査 R16)。時間は役割で 9 段、出どころは `app.css` の `--dur-*` と `DUR` だけで値を一致させ、画面側に `0.18s` / `220` を直書きしない(R15)。押せる面の `transition` は `app.css` の既定に任せ、部品側で書き足さない
    - **開閉は 3 方式、選び方は §10「開閉の 3 方式」。** 既定は `<details>`(キーボード・ページ内検索の自動展開をブラウザが持つ)、面・キャレット・トリガが別要素なら `use:disclosurePane` + `use:disclosureCaret`(R17)、`flex` の子で `slide` が効かないなら `use:collapse`。キャレットは `.caret` + `.rot` で回す(▸▾ の文字差し替えは使わない — 動きが出ず開閉が一瞬わからない)。`scrollIntoView` は `smooth` を直書きせず `reveal()` を通す(R19)。**新しい動きを足したら、動きを消す設定(prefers-reduced-motion)で実機で本当に消えるか確かめる**(§10 に実測済みの落とし穴の記録あり)
