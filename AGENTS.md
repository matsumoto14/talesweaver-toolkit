...
- **押した場所は動かない**。クリックした要素の上に何も差し込まない。ドリルダウンは置き換えず右にペインを増やす。重なるもの(候補・ポップオーバー)はレイアウトを押さない。数値欄は `min-width` + `tabular-nums` で桁が増えても幅が変わらない
    ...
    - **動きの時間は役割で 9 段**(§10「時間の段」)。出どころは `app.css` の `--dur-*` と `ui/motion.svelte.ts` の `DUR` だけで、値は一致させる。画面側に `0.18s` / `220` を直書きしない(機械監査 R15)。押せる面の `transition` は `app.css` の既定に任せ、部品側で書き足さない。開閉のキャレットは `.caret` + `.rot` で回す(▸▾ の差し替えは使わない)
