...
- **押した場所は動かない**。クリックした要素の上に何も差し込まない。ドリルダウンは置き換えず右にペインを増やす。重なるもの(候補・ポップオーバー)はレイアウトを押さない。数値欄は `min-width` + `tabular-nums` で桁が増えても幅が変わらない
    ...
    - **動きは「何が変わったか」を認知させるためだけに使う**。変わった要素だけ動かす。同時に変わるものは全部動かす。増減の色は必ず元に戻す。0.5s を超えない。§10 は「目的から探す」の入口表から始まる — `--dur-*` や `use:*` の名前ではなく「これを作った、ユーザーに何を気づかせたいか」から引く
    - **その要素がどう動くかは入場クラス 1 つ**(`.pane-in` / `.swap-in` / `.pop-in` / `.badge-in` / `.open-in`)**が決め、いつ動かすかだけを部品が持つ**(§10「動きの部品」)。**値を出すときは `ui/Value.svelte`**(§08「値は `<Value>` 1 つ」)— 数値書体・未収録の `?`・跳ね・差分枠を持つので、画面側は `use:bump` / `use:delta` を呼ばない。`use:changed` を直接呼ぶのは、値ではなく面・行・文が変わったときだけ(その要素が載せている入場クラスをもう一度再生する。無ければ `.badge-in`)。「クラスを画面側で手で当てない」は 2026-09-17 に取り下げ — 入場クラスは手で当てた動きではなく、部品が読む**動き方の宣言**になったため(R16 も同日廃止。部品が動きを持った時点で、規則が指す先が部品自身になった)。時間は役割で 9 段、出どころは `app.css` の `--dur-*` と `DUR` だけで値を一致させ、画面側に `0.18s` / `220` を直書きしない(R15 は 2026-09-17 に廃止したので機械は見張っていない)。押せる面の `transition` は `app.css` の既定に任せ、部品側で書き足さない
    - **押すと開く面は `ui/Disclosure.svelte` だけで作る**(§10「開閉は `<Disclosure>` 1 つ」)。土台は `<details>` で、キーボード・ページ内検索の自動展開・開閉の状態・読み上げをブラウザが持つ。**`aria-expanded` は書かない**(`<summary>` が既に持っている)。キャレットも部品が 1 つだけ置いて回すので、向きも位置も画面側では決めない。排他は `group`(`<details name>`)。トリガと面が別の入れ物にいて `<details>` に載らない場面(計算タブの鎖・「なぜこの数字?」・テシスコアの補助タイプ)だけは、面に `class="… open-in"` + `hidden={!open}` を直接書く。`scrollIntoView` は `smooth` を直書きせず `reveal()` を通す(R19)。**新しい動きを足したら、動きを消す設定(prefers-reduced-motion)で実機で本当に消えるか確かめる**(§10 に実測済みの落とし穴の記録あり)
